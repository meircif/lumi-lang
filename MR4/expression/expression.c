...
Returncode Expression_parse_new(Expression* self, String* ends, SyntaxTreeCode* code_node, Operator* curr_operator, Expression** expression, Char* end, Operator** operator) {
  ...
  if (NULL != (*operator)) {
    CHECK(51, UnaryExpression_parse_new(NULL, ends, (*operator), code_node, &((*expression)), &((*end)), &((*operator))) )
  }
  else {
    CHECK(55, Expression_parse_new_operand(self, text, ends, code_node, &((*expression)), &((*end))) )
    if ((*end) == ' ') {
      CHECK(58, Expression_read_new_value(self, ends, &(text), &((*end))) )
      CHECK(59, NameMap_find(glob->operator_map, text, (void**)&((*operator))) )
      if (!(NULL != (*operator))) {
        CHECK(61, SyntaxTreeNode_m_syntax_error(&(code_node->_base), &(String){17, 16, "unknown operator"}, text) )
      }
    }
  }
  ...
  while (true) {
    ...
    CHECK(67, BinaryExpression_parse_new(NULL, ends, (*operator), code_node, (*expression), binary_left_expression, &(new_binary_expression), &((*end)), &((*operator))) )
...
Returncode Expression_read_new_value(Expression* self, String* ends, String** text, Char* end) {
  ...
  CHECK(85, string_new_concat(&(String){7, 6, " .[({?"}, ends, &(all_ends)) )
  CHECK(86, read_new(all_ends, &((*text)), &((*end))) )
  free(all_ends);
  return OK;
}
#undef MR_FUNC_NAME
...
Returncode Expression_parse_new_operand(Expression* self, String* text, String* ends, SyntaxTreeCode* code_node, Expression** expression, Char* end) {
  if (text->length == 0) {
    if ((*end) == '(') {
      CHECK(95, BlockExpression_parse_new(NULL, code_node, &((*expression)), &((*end))) )
    }
    else {
      CHECK(98, SyntaxTreeNode_m_syntax_error_c(&(code_node->_base), &(String){11, 10, "unexpected"}, (*end)) )
    }
    free(text);
  }
  else {
    CHECK(101, Expression_parse_new_init_operand(self, text, code_node, &((*expression)), &((*end))) )
  }
  ...
  while (true) {
    ...
    Bool _Bool39;
    CHECK(107, Expression_parse_new_follow_operand(self, ends, code_node, &((*expression)), &((*end)), &(_Bool39)) )
    if (!(_Bool39)) break;
  }
  return OK;
}
...
Returncode Expression_parse_new_init_operand(Expression* self, String* text, SyntaxTreeCode* code_node, Expression** expression, Char* end) {
  if ((0) < 0 || (0) >= (text)->length) RAISE(114)
  Char first = ((text)->values[0]);
  Char second = '\0';
  if (text->length > 1) {
    if ((1) < 0 || (1) >= (text)->length) RAISE(117)
    second = ((text)->values[1]);
  }
  if (first == '\'') {
    CHECK(119, CharExpression_parse_new(NULL, text, &((*expression))) )
  }
  else {
    if (first == '"') {
      CHECK(121, StringExpression_parse_new(NULL, text, code_node, &((*expression))) )
    }
    else {
      if ((first >= '0' && first <= '9') || (first == '-' && second >= '0' && second <= '9')) {
        CHECK(125, IntExpression_parse_new(NULL, text, &((*expression))) )
      }
      else {
        if (first >= 'A' && first <= 'Z' && second >= 'a' && second <= 'z') {
          CHECK(127, TypeExpression_parse_new(NULL, text, code_node, &((*expression)), &((*end))) )
        }
        else {
          if (text->length == 1 && first == '_') {
            ...
            Bool _Bool40;
            CHECK(131, String_equal(text, &(String){5, 4, "base"}, &(_Bool40)) )
            if (_Bool40) {
...
Returncode Expression_add_aux_variable(Expression* self, Int access, Bool is_create, TypeInstance* type_instance, SymbolExpression** symbol) {
  ...
  *(*symbol) = (SymbolExpression){SymbolExpression__dtl, NULL, 0, NULL, NULL, 0, false, false, false, NULL, NULL, NULL};
  ...
  CHECK(167, SyntaxTreeCode_get_function(self->code_node, &(_SyntaxTreeFunction41)) )
  CHECK(167, SyntaxTreeFunction_add_aux_variable(_SyntaxTreeFunction41, access, is_create, type_instance, &((*symbol)->variable)) )
...
Returncode Expression_get_parent_type(Expression* self, TypeData** parent_type) {
  if (NULL != self->code_node) {
    CHECK(173, (self->code_node)->_base._dtl[0](self->code_node, &((*parent_type))) )
  }
  else {
    CHECK(175, SyntaxTreeNode_get_parent_type(&(self->_base), &((*parent_type))) )
  }
  return OK;
}
#undef MR_FUNC_NAME
...
Returncode Expression_write_refman_init(Expression* self, SymbolExpression* symbol) {
  ...
  CHECK(181, (symbol)->_base._base._dtl[3](symbol) )
  CHECK(182, write(&(String){22, 21, "_Refman = MR_new_ref("}) )
  CHECK(183, (symbol)->_base._base._dtl[3](symbol) )
  CHECK(184, write(&(String){4, 3, ");\n"}) )
  CHECK(185, SyntaxTreeCode_write_spaces(self->code_node) )
  CHECK(186, write(&(String){5, 4, "if ("}) )
  CHECK(187, (symbol)->_base._base._dtl[3](symbol) )
  CHECK(188, write(&(String){18, 17, "_Refman == NULL) "}) )
  CHECK(189, SyntaxTreeNode_write_raise(&(self->_base)) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
...
Returncode Expression_write_init_var_ref(Expression* self, SymbolExpression* symbol) {
  /* `symbol` = &`symbol`_Var; */
  CHECK(193, (symbol)->_base._base._dtl[3](symbol) )
  CHECK(194, write(&(String){5, 4, " = &"}) )
  CHECK(195, (symbol)->_base._base._dtl[3](symbol) )
  CHECK(196, write(&(String){7, 6, "_Var;\n"}) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
...
Returncode Expression_write_validate_ref(Expression* self) {
  CHECK(199, Expression_write_as_top(self) )
  CHECK(200, write(&(String){9, 8, " == NULL"}) )
  if (!self->result_type->type_data->is_primitive) {
    CHECK(202, write(&(String){5, 4, " || "}) )
    Bool top = self->top;
    self->top = false;
    CHECK(205, (self)->_base._dtl[5](self) )
    self->top = top;
    CHECK(207, write(&(String){16, 15, "->value == NULL"}) )
  }
  return OK;
}
#undef MR_FUNC_NAME
...
Returncode Expression_write_dynamic(Expression* self) {
  CHECK(210, (self)->_base._dtl[3](self) )
...
Returncode Expression_write_refman(Expression* self) {
  CHECK(214, (self)->_base._dtl[3](self) )
...
Returncode Expression_write_as_top(Expression* self) {
  Bool top = self->top;
  self->top = true;
  CHECK(220, (self)->_base._dtl[3](self) )
  self->top = top;
  return OK;
}
#undef MR_FUNC_NAME
