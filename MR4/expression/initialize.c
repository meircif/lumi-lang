...
Returncode InitExpression_analyze(InitExpression* self) {
  ...
  if (NULL != self->symbol) {
    ...
    CHECK(29, Expression_add_aux_variable(&(self->_base), ACCESS_NEW, self->_base.result_type, &(self->symbol)) )
  }
  
  if (self->symbol->variable->access == ACCESS_NEW && self->_base.result_type->type_data->is_primitive) {
    CHECK(34, SyntaxTreeNode_m_syntax_error(&(self->_base._base), &(String){37, 36, "dynamic allocation of primitive type"}, self->_base.result_type->type_data->name) )
  }
  ...
  if (!self->_base.result_type->type_data->is_primitive && (self->symbol->variable->access == ACCESS_VAR || self->symbol->variable->access == ACCESS_NEW)) {
    CHECK(41, TypeInstance_check_sequence(self->_base.result_type, &(self->_base._base)) )
    Int _Int35;
    CHECK(42, TypeData_find_meth(self->_base.result_type->type_data, &(String){4, 3, "new"}, &(self->constructor), &(_Int35)) )
    if (!(NULL != self->arguments->parameters->first) &&  ! (NULL != self->arguments->outputs->first) && (!(NULL != self->constructor) || self->_base.result_type->type_data == &(glob->type_string->_base))) {
      ...
      if (!(NULL != self->constructor)) {
        CHECK(48, SyntaxTreeNode_m_syntax_error(&(self->_base._base), &(String){23, 22, "no contructor for type"}, self->_base.result_type->type_data->name) )
      }
      else {
        ...
        *self_param = (CallArgument){CallArgument__dtl, NULL, 0, 0, false, NULL, NULL, false, false};
        ...
        Bool _Bool36;
        CHECK(57, FunctionArguments_check_same_as(self->arguments, self->constructor->arguments, &(_Bool36)) )
      }
    }
    ...
    if (NULL != self->arguments->parameters->first) {
      if (NULL != self->arguments->parameters->first->next ||  NULL !=  self->arguments->outputs->first) {
        CHECK(62, SyntaxTreeNode_m_syntax_error_msg(&(self->_base._base), &(String){43, 42, "only one initialization parameter expected"}) )
      }
      Argument* param = self->arguments->parameters->first->item;
      CHECK(65, (param)->_base._dtl[6](param, self->_base.result_type) )
    }
  }
  return OK;
}
...
Returncode InitExpression_write_allocation(InitExpression* self) {
  CHECK(68, FunctionArguments_write_preactions(self->arguments) )
  if (self->_base.result_type->type_data == &(glob->type_array->_base) || self->_base.result_type->type_data == &(glob->type_string->_base)) {
    CHECK(71, (self->_base.result_type->length)->_base._dtl[6](self->_base.result_type->length) )
  }
  if (self->_base.result_type->type_data == &(glob->type_array->_base) && self->_base.result_type->sub_type->type_data == &(glob->type_string->_base)) {
    CHECK(74, (self->_base.result_type->sub_type->length)->_base._dtl[6](self->_base.result_type->sub_type->length) )
  }
  
  if (self->symbol->variable->access == ACCESS_VAR) {
    CHECK(77, InitExpression_write_var_init(self) )
  }
  else {
    CHECK(79, InitExpression_write_new_init(self) )
  }
  
  CHECK(81, Expression_write_refman_init(&(self->_base), self->symbol) )
  
  if (NULL != self->constructor) {
    CHECK(84, SyntaxTreeCode_write_spaces(self->_base.code_node) )
    CHECK(85, SyntaxTreeNode_write_call(&(self->_base._base)) )
    CHECK(86, SyntaxTreeFunction_write_cname(self->constructor) )
    CHECK(87, (self->arguments)->_base._dtl[3](self->arguments) )
...
Returncode InitExpression_write_var_init(InitExpression* self) {
  CHECK(91, Expression_write_init_var_ref(&(self->_base), self->symbol) )
  
  if (self->_base.result_type->type_data == &(glob->type_array->_base) || self->_base.result_type->type_data == &(glob->type_string->_base)) {
    /* `symbol`_Var.values = `symbol`_Values; */
    CHECK(96, SyntaxTreeCode_write_spaces(self->_base.code_node) )
    CHECK(97, (self->symbol)->_base._base._dtl[2](self->symbol) )
    CHECK(98, write(&(String){15, 14, "_Var.values = "}) )
    CHECK(99, (self->symbol)->_base._base._dtl[2](self->symbol) )
    CHECK(100, write(&(String){10, 9, "_Values;\n"}) )
    if (self->_base.result_type->type_data == &(glob->type_array->_base) && self->_base.result_type->sub_type->type_data == &(glob->type_string->_base)) {
      ...
      CHECK(106, write(&(String){25, 24, "MR_set_var_string_array("}) )
      CHECK(107, (self->_base.result_type->length)->_base._dtl[2](self->_base.result_type->length) )
      CHECK(108, write(&(String){3, 2, ", "}) )
      CHECK(109, (self->_base.result_type->sub_type->length)->_base._dtl[2](self->_base.result_type->sub_type->length) )
      CHECK(110, write(&(String){3, 2, ", "}) )
      CHECK(111, (self->symbol)->_base._base._dtl[2](self->symbol) )
      CHECK(112, write(&(String){3, 2, ", "}) )
      CHECK(113, (self->symbol)->_base._base._dtl[2](self->symbol) )
      CHECK(114, write(&(String){10, 9, "_Chars);\n"}) )
    }
  }
  return OK;
}
...
Returncode InitExpression_write_new_init(InitExpression* self) {
  /* `symbol` = ... */
  CHECK(118, (self->symbol)->_base._base._dtl[2](self->symbol) )
  ...
  if (self->_base.result_type->type_data == &(glob->type_array->_base)) {
    if (self->_base.result_type->sub_type->type_data == &(glob->type_string->_base)) {
      ...
      CHECK(124, write(&(String){21, 20, "MR_new_string_array("}) )
      CHECK(125, (self->_base.result_type->length)->_base._dtl[2](self->_base.result_type->length) )
      CHECK(126, write(&(String){3, 2, ", "}) )
      CHECK(127, (self->_base.result_type->sub_type->length)->_base._dtl[2](self->_base.result_type->sub_type->length) )
      
    }
    else {
      /* MR_new_array(`length`, sizeof(`SubType`)); */
      CHECK(131, write(&(String){14, 13, "MR_new_array("}) )
      CHECK(132, (self->_base.result_type->length)->_base._dtl[2](self->_base.result_type->length) )
      CHECK(133, write(&(String){10, 9, ", sizeof("}) )
      CHECK(134, write_cname(self->_base.result_type->sub_type->type_data->name) )
    ...
    if (self->_base.result_type->type_data == &(glob->type_string->_base)) {
      ...
      CHECK(139, write(&(String){15, 14, "MR_new_string("}) )
      CHECK(140, (self->_base.result_type->length)->_base._dtl[2](self->_base.result_type->length) )
      
    }
    else {
      /* calloc(1, sizeof(`type`)); */
      CHECK(144, write(&(String){18, 17, "calloc(1, sizeof("}) )
      CHECK(145, write_cname(self->_base.result_type->type_data->name) )
  ...
  CHECK(151, SyntaxTreeCode_write_spaces(self->_base.code_node) )
  CHECK(152, write(&(String){5, 4, "if ("}) )
  CHECK(153, (self->symbol)->_base._base._dtl[2](self->symbol) )
  CHECK(154, write(&(String){11, 10, " == NULL) "}) )
  CHECK(155, SyntaxTreeNode_write_raise(&(self->_base._base)) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
...
Returncode InitExpression_write_assign(InitExpression* self) {
  ...
  CHECK(160, (((Argument*)(self->arguments->parameters->first->item)))->_base._dtl[8](((Argument*)(self->arguments->parameters->first->item)), &(value)) )
  CHECK(161, SyntaxTreeCode_write_spaces(self->_base.code_node) )
  CHECK(162, (value)->_base._dtl[6](value) )
  CHECK(163, (self->symbol)->_base._base._dtl[2](self->symbol) )
  CHECK(164, write(&(String){4, 3, " = "}) )
  CHECK(165, (value)->_base._dtl[2](value) )
