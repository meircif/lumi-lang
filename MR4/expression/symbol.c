...
Returncode SymbolExpression_parse_new(SymbolExpression* self, String* text, SyntaxTreeCode* code_node, Expression** expression) {
  ...
  *symbol_expression = (SymbolExpression){SymbolExpression__dtl, NULL, 0, NULL, NULL, 0, false, false, false, false, false, NULL, NULL, NULL, false};
...
Returncode SymbolExpression_analyze(SymbolExpression* self) {
  ...
  if (NULL != self->variable) {
    ...
    CHECK(29, SyntaxTreeNamespace_find_function(&(glob->root->_base), self->name, &(self->function)) )
    if (!(NULL != self->function)) {
      CHECK(31, SyntaxTreeNode_m_syntax_error(&(self->_base._base), &(String){15, 14, "unknown symbol"}, self->name) )
    }
    CHECK(32, Expression_set_simple_type(&(self->_base), &(glob->type_func->_base)) )
    self->_base.access = ACCESS_VAR;
    CHECK(34, FunctionArguments_copy_new(self->function->arguments, &(self->_base.result_type->arguments)) )
  }
  return OK;
}
#undef MR_FUNC_NAME
...
Returncode SymbolExpression_write(SymbolExpression* self) {
  if (NULL != self->function) {
    if (NULL != self->function->mocker_function &&  ! self->is_annotate) {
      CHECK(52, SyntaxTreeFunction_write_cname(&(self->function->mocker_function->_base)) )
    }
    else {
...
Returncode SymbolExpression_write_dynamic(SymbolExpression* self) {
  if (NULL != self->variable && self->variable->is_output &&  ! self->_base.top) {
    ...
    CHECK(68, write(&(String){10, 9, "_Dynamic)"}) )
  }
  else {
    CHECK(70, Expression_write_dynamic(&(self->_base)) )
  }
  return OK;
}
#undef MR_FUNC_NAME
...
Returncode SymbolExpression_write_refman(SymbolExpression* self) {
  if (NULL != self->variable && self->variable->is_output &&  ! self->_base.top) {
    ...
    CHECK(76, write(&(String){9, 8, "_Refman)"}) )
  }
  else {
    CHECK(78, Expression_write_refman(&(self->_base)) )
  }
  return OK;
}
#undef MR_FUNC_NAME
...
Func SymbolExpression__dtl[] = {(void*)Expression_get_parent_type, (void*)SyntaxTreeNode_link_types, (void*)SymbolExpression_analyze, (void*)SymbolExpression_write, (void*)SymbolExpression_write_dynamic, (void*)SymbolExpression_write_refman, (void*)SymbolExpression_analyze_call, (void*)SymbolExpression_analyze_mock, (void*)Expression_write_preactions};
#endif
...
Returncode MemberExpression_parse_new(MemberExpression* self, String* ends, SyntaxTreeCode* code_node, Expression** expression, Char* end) {
  ...
  if (member_expression == NULL) RAISE(89)
  *member_expression = (MemberExpression){MemberExpression__dtl, NULL, 0, NULL, NULL, 0, false, false, false, false, false, NULL, NULL, NULL, false, NULL, NULL, 0, false};
  member_expression->_base._base._base._dtl = MemberExpression__dtl;
  CHECK(90, MemberExpression_parse(member_expression, (*expression), ends, code_node, &((*end))) )
...
Returncode MemberExpression_parse(MemberExpression* self, Expression* instance, String* ends, SyntaxTreeCode* code_node, Char* end) {
  ...
  CHECK(102, SyntaxTreeNode_set_location(&(self->_base._base._base)) )
  CHECK(103, Expression_read_new_value(&(self->_base._base), ends, &(self->_base.name), &((*end))) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
...
Returncode MemberExpression_analyze(MemberExpression* self) {
  CHECK(106, (self->instance)->_base._dtl[2](self->instance) )
  ...
  if (!(NULL != instance_type) ||  ! (NULL != instance_type->type_data)) {
    CHECK(110, SyntaxTreeNode_m_syntax_error(&(self->_base._base._base), &(String){30, 29, "void expression has no member"}, self->_base.name) )
  }
  ...
  CHECK(116, TypeData_find_field(instance_type->type_data, self->_base.name, &(self->_base.variable), &(self->bases)) )
  if (NULL != self->_base.variable) {
    ...
    CHECK(123, TypeInstance_f_new_replace_params(self->_base.variable->type_instance, instance_type, self->bases, &(self->_base._base.result_type)) )
