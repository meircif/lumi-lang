...
Returncode MemberExpression_parse_new(MemberExpression* self, String* ends, SyntaxTreeCode* code_node, Expression** expression, Char* end) {
  ...
  if (member_expression == NULL) RAISE(102)
  *member_expression = (MemberExpression){MemberExpression__dtl, NULL, 0, NULL, NULL, false, false, false, NULL, NULL, NULL, NULL, 0};
  ...
  CHECK(103, MemberExpression_parse(member_expression, (*expression), ends, code_node, &((*end))) )
...
Returncode MemberExpression_parse(MemberExpression* self, Expression* instance, String* ends, SyntaxTreeCode* code_node, Char* end) {
  ...
  CHECK(115, SyntaxTreeNode_set_location(&(self->_base._base._base)) )
  CHECK(116, Expression_read_new_value(&(self->_base._base), ends, &(self->_base.name), &((*end))) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
...
Returncode MemberExpression_analyze(MemberExpression* self) {
  CHECK(119, (self->instance)->_base._dtl[1](self->instance) )
  ...
  if (!(NULL != self->instance->result_type) ||  ! (NULL != self->instance->result_type->type_data)) {
    CHECK(123, SyntaxTreeNode_m_syntax_error(&(self->_base._base._base), &(String){30, 29, "void expression has no member"}, self->_base.name) )
  }
  ...
  CHECK(130, TypeData_m_find_field(type_data, self->_base.name, &(self->_base.variable), &(self->bases)) )
  if (NULL != self->_base.variable) {
    CHECK(132, TypeInstance_m_copy_new(self->_base.variable->type_instance, &(self->_base._base.result_type)) )
    self->_base._base.assignable = true;
  }
  else {
    CHECK(135, TypeData_m_find_meth(type_data, self->_base.name, &(self->_base.function), &(self->bases)) )
    if (!(NULL != self->_base.function)) {
      CHECK(138, SyntaxTreeNode_m_syntax_error2(&(self->_base._base._base), &(String){5, 4, "type"}, type_data->name, &(String){14, 13, "has no member"}, self->_base.name) )
    }
    CHECK(143, Expression_set_simple_type(&(self->_base._base), glob->type_func) )
    CHECK(144, FunctionArguments_m_copy_new(self->_base.function->arguments, &(self->_base._base.result_type->arguments)) )
  }
  if (self->instance->result_type->type_data == glob->type_base) {
    if (!(NULL != self->_base.function)) {
      CHECK(147, SyntaxTreeNode_m_syntax_error(&(self->_base._base._base), &(String){31, 30, "calling \"base\" with non-method"}, self->_base.name) )
    }
...
Returncode MemberExpression_analyze_call(MemberExpression* self, FunctionArguments* arguments) {
  if (self->instance->result_type->type_data == glob->type_type) {
    ...
    *self_param = (CallArgument){CallArgument__dtl, NULL, 0, 0, false, NULL, NULL, false};
    ...
    CHECK(161, List_m_prepend(arguments->parameters, &(self_param->_base)) )
  }
  self->bases = 0;
  self->instance = NULL;
  return OK;
...
Returncode MemberExpression_write_preactions(MemberExpression* self) {
  ...
  CHECK(168, (self->instance)->_base._dtl[4](self->instance) )
  if (self->instance->result_type->type_data != glob->type_type) {
    /* if (`instance` == NULL) RAISE(`line-num`) */
    CHECK(171, write(&(String){5, 4, "if ("}) )
    CHECK(172, (self->instance)->_base._dtl[2](self->instance) )
    CHECK(173, write(&(String){11, 10, " == NULL) "}) )
    CHECK(174, SyntaxTreeNode_write_raise(&(self->_base._base._base)) )
    CHECK(175, write(&(String){2, 1, "\n"}) )
    CHECK(176, SyntaxTreeCode_write_spaces(self->_base._base.code_node) )
  }
  return OK;
}
#undef MR_FUNC_NAME
...
Returncode MemberExpression_write(MemberExpression* self) {
  if (NULL != self->instance) {
    ...
    {int n; for (n = (0); n < (self->bases); ++n) {
      CHECK(183, write(&(String){7, 6, "_base."}) )
    }}
  }
  CHECK(184, SymbolExpression_write(&(self->_base)) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
...
Returncode UpCastExpression_init_new(UpCastExpression* self, Int bases, Expression** expression) {
  ...
  *up_cast = (UpCastExpression){UpCastExpression__dtl, NULL, 0, NULL, NULL, false, false, false, NULL, 0};
  ...
  CHECK(194, UpCastExpression_init(up_cast, bases, (*expression)) )
...
Returncode UpCastExpression_init(UpCastExpression* self, Int bases, Expression* expression) {
  ...
  self->bases = bases;
  CHECK(200, TypeInstance_m_copy_new(self->expression->result_type, &(self->_base.result_type)) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
...
Returncode UpCastExpression_write_preactions(UpCastExpression* self) {
  CHECK(203, (self->expression)->_base._dtl[4](self->expression) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
...
Returncode UpCastExpression_write(UpCastExpression* self) {
  ...
  CHECK(208, (self->expression)->_base._dtl[2](self->expression) )
  CHECK(209, write(&(String){8, 7, "->_base"}) )
  {int n; for (n = (1); n < (self->bases); ++n) {
    CHECK(211, write(&(String){7, 6, "._base"}) )
  }}
