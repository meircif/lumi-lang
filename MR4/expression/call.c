...
Returncode CallExpression_write_preactions(CallExpression* self) {
  CHECK(54, (self->function)->_base._dtl[5](self->function) )
  if (self->is_function_object) {
    CHECK(56, write(&(String){5, 4, "if ("}) )
    self->function->top = true;
    CHECK(58, (self->function)->_base._dtl[2](self->function) )
    self->function->top = false;
    CHECK(60, write(&(String){11, 10, " == NULL) "}) )
    CHECK(61, SyntaxTreeNode_write_raise(&(self->_base._base)) )
    CHECK(62, SyntaxTreeCode_write_spaces(self->_base.code_node) )
  }
  CHECK(63, FunctionArguments_write_preactions(self->arguments) )
  if (!self->_base.is_statement) {
    CHECK(65, CallExpression_write_func_call(self) )
...
Returncode CallExpression_write(CallExpression* self) {
  if (self->_base.is_statement) {
    CHECK(70, CallExpression_write_func_call(self) )
  }
  else {
    if (NULL != self->output) {
      CHECK(72, (self->output)->_base._dtl[2](self->output) )
    }
    else {
      RAISE(74)
    }
...
Returncode CallExpression_write_func_call(CallExpression* self) {
  CHECK(77, SyntaxTreeNode_write_call(&(self->_base._base)) )
  CHECK(78, (self->function)->_base._dtl[2](self->function) )
  CHECK(79, (self->arguments)->_base._dtl[3](self->arguments) )
  CHECK(80, write(&(String){4, 3, " )\n"}) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
...
Returncode CallArgument_parse_value(CallArgument* self, SyntaxTreeCode* code_node, Char* end) {
  ...
  CHECK(93, parse_new_expression(&(String){3, 2, ",)"}, code_node, &(self->value), &((*end))) )
  if ((*end) != ',' && (*end) != ')') {
    CHECK(96, SyntaxTreeNode_m_syntax_error_c(&(self->_base._base), &(String){25, 24, "expected \",\" or \")\", got"}, (*end)) )
  }
  return OK;
}
#undef MR_FUNC_NAME
...
Returncode CallArgument_analyze(CallArgument* self) {
  CHECK(99, (self->value)->_base._dtl[1](self->value) )
  if (self->_base.is_output &&  ! self->value->assignable) {
    CHECK(101, SyntaxTreeNode_m_syntax_error_msg(&(self->_base._base), &(String){27, 26, "non assignable call output"}) )
  }
  return OK;
}
#undef MR_FUNC_NAME
...
Returncode CallArgument_check_same_type_as(CallArgument* self, TypeInstance* type_instance) {
  if (self->_base.is_output) {
    Int _Int16;
    CHECK(105, TypeInstance_check_assign_to(type_instance, self->value->result_type, &(self->_base._base), &(_Int16)) )
    self->is_down_cast = _Int16 > 0;
  }
  else {
    CHECK(108, TypeInstance_check_assign_from(type_instance, &(self->_base._base), &(self->value)) )
  }
...
Returncode CallArgument_write_preactions(CallArgument* self) {
  CHECK(115, (self->value)->_base._dtl[5](self->value) )
  if (self->is_down_cast) {
    /* if (`value` != NULL) RAISE(`line-num`) */
    CHECK(118, write(&(String){5, 4, "if ("}) )
    CHECK(119, (self->value)->_base._dtl[2](self->value) )
    CHECK(120, write(&(String){11, 10, " != NULL) "}) )
    CHECK(121, SyntaxTreeNode_write_raise(&(self->_base._base)) )
    CHECK(122, SyntaxTreeCode_write_spaces(self->code_node) )
  }
  return OK;
}
#undef MR_FUNC_NAME
...
Returncode CallArgument_write(CallArgument* self) {
  ...
  if (self->_base.is_output) {
    if (self->is_down_cast) {
      CHECK(128, write(&(String){8, 7, "(void*)"}) )
    }
    CHECK(129, write(&(String){3, 2, "&("}) )
  }
  CHECK(130, (self->value)->_base._dtl[2](self->value) )
  if (self->_base.is_output) {
  ...
  if (self->is_dynamic) {
    ...
    if (self->_base.is_output) {
      if (self->is_down_cast) {
        CHECK(137, write(&(String){8, 7, "(void*)"}) )
      }
      CHECK(138, write(&(String){3, 2, "&("}) )
    }
    CHECK(139, (self->value)->_base._dtl[3](self->value) )
    if (self->_base.is_output) {
...
Returncode CallArgumentFactory_m_new_argument(CallArgumentFactory* self, Argument** new_argument) {
  ...
  if (_CallArgument17 == NULL) RAISE(146)
