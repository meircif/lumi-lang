...
Returncode SliceExpression_write(SliceExpression* self) {
  ...
  if (self->sequence->result_type->type_data != &(glob->type_string->_base)) {
    if (self->_base.result_type->type_data == &(glob->type_func->_base) &&  NULL !=  self->_base.result_type->arguments) {
      CHECK(138, FunctionArguments_write_pointer(self->_base.result_type->arguments, &(String){2, 1, "*"}) )
    }
    else {
      CHECK(140, write_cname(self->_base.result_type->type_data->name) )
  ...
  CHECK(145, (self->sequence)->_base._dtl[2](self->sequence) )
  CHECK(146, write(&(String){11, 10, ")->values)"}) )
  if (self->sequence->result_type->type_data != &(glob->type_string->_base)) {
    CHECK(148, write(&(String){2, 1, ")"}) )
  }
  CHECK(149, write(&(String){2, 1, "["}) )
  CHECK(150, (self->index)->_base._dtl[2](self->index) )
...
Returncode SliceExpression_write_dynamic(SliceExpression* self) {
  ...
  CHECK(157, write_cname(self->_base.result_type->type_data->name) )
  CHECK(158, write(&(String){9, 8, "_dynamic"}) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
