...
Returncode test_dynamic() {
  ...
  CHECK(581, test_code(&(String){9, 8, "new Ta a"}, &(String){82, 81, "Ta* a = NULL;\n  Ta_Dynamic* a_Dynamic = &Ta_dynamic;\n  a = calloc(1, sizeof(Ta));"}) )
  CHECK(584, test_code(&(String){14, 13, "user Ta a(ta)"}, &(String){82, 81, "Ta* a = NULL;\n  Ta_Dynamic* a_Dynamic = NULL;\n  a = ta;\n  a_Dynamic = ta_Dynamic;"}) )
  CHECK(587, test_code(&(String){9, 8, "ta := ta"}, &(String){36, 35, "ta_Dynamic = ta_Dynamic;\n  ta = ta;"}) )
  CHECK(588, test_code(&(String){9, 8, "ta := tb"}, &(String){56, 55, "ta_Dynamic = &(tb_Dynamic->_base);\n  ta = &(tb->_base);"}) )
  CHECK(591, test_code(&(String){9, 8, "ta := tc"}, &(String){68, 67, "ta_Dynamic = &(tc_Dynamic->_base._base);\n  ta = &(tc->_base._base);"}) )
  CHECK(594, test_code(&(String){8, 7, "ta := _"}, &(String){32, 31, "ta_Dynamic = NULL;\n  ta = NULL;"}) )
  CHECK(595, test_code(&(String){32, 31, "user Array{Ta} aa\n  ta := aa[4]"}, &(String){127, 126, "Array* aa = NULL;\n  if ((4) < 0 || (4) >= (aa)->length) RAISE(2)\n  ta_Dynamic = &Ta_dynamic;\n  ta = ((Ta**)((aa)->values))[4];"}) )
  CHECK(598, test_code(&(String){32, 31, "user Array{Tc} ca\n  ta := ca[4]"}, &(String){161, 160, "Array* ca = NULL;\n  if ((4) < 0 || (4) >= (ca)->length) RAISE(2)\n  ta_Dynamic = &(&Tc_dynamic->_base._base);\n  ta = &((((Tc**)((ca)->values))[4])->_base._base);"}) )
  CHECK(601, test_code(&(String){9, 8, "ta.dyn()"}, &(String){78, 77, "if (ta_Dynamic == NULL) RAISE(1)\n  CHECK(1, ta_Dynamic->dyn(ta, ta_Dynamic) )"}) )
  CHECK(604, test_code(&(String){9, 8, "tb.dyn()"}, &(String){104, 103, "if (tb_Dynamic == NULL) RAISE(1)\n  CHECK(1, tb_Dynamic->_base.dyn(&(tb->_base), &(tb_Dynamic->_base)) )"}) )
  CHECK(607, test_code(&(String){9, 8, "tc.dyn()"}, &(String){122, 121, "if (tc_Dynamic == NULL) RAISE(1)\n  CHECK(1, tc_Dynamic->_base._base.dyn(&(tc->_base._base), &(tc_Dynamic->_base._base)) )"}) )
  CHECK(610, test_code(&(String){18, 17, "fun7()->(user ta)"}, &(String){79, 78, "if (ta != NULL) RAISE(1)\n  CHECK(1, fun7((void*)&(ta), (void*)&(ta_Dynamic)) )"}) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
...
Returncode test_function_object() {
  CHECK(616, test_code(&(String){39, 38, "var Func{()} fun\n  fun := fun0\n  fun()"}, &(String){93, 92, "Returncode (*fun)(void) = NULL;\n  fun = fun0;\n  if (fun == NULL) RAISE(3)\n  CHECK(3, fun() )"}) )
  CHECK(619, test_code(&(String){83, 82, "var Func{(copy Int x)->(copy Int y)} fun(fun5)\n  var Int x\n  fun(copy 9)->(copy x)"}, &(String){122, 121, "Returncode (*fun)(Int x, Int* y) = NULL;\n  Int x = 0;\n  fun = fun5;\n  if (fun == NULL) RAISE(3)\n  CHECK(3, fun(9, &(x)) )"}) )
  CHECK(622, test_code(&(String){54, 53, "var Func{(var Test self)} fun(Test.meth)\n  fun(var t)"}, &(String){105, 104, "Returncode (*fun)(Test* self) = NULL;\n  fun = Test_meth;\n  if (fun == NULL) RAISE(2)\n  CHECK(2, fun(t) )"}) )
  CHECK(625, test_code(&(String){40, 39, "var Func{()} fun\n  fun := _\n  b := fun?"}, &(String){65, 64, "Returncode (*fun)(void) = NULL;\n  fun = NULL;\n  b = fun != NULL;"}) )
  CHECK(628, test_code(&(String){24, 23, "t.fun := fun0\n  t.fun()"}, &(String){120, 119, "if (t == NULL) RAISE(1)\n  t->fun = fun0;\n  if (t == NULL) RAISE(2)\n  if (t->fun == NULL) RAISE(2)\n  CHECK(2, t->fun() )"}) )
  String* expected = &(String){1024, 0, (char[1024]){0}};
  CHECK(632, String_copy(expected, &(String){38, 37, "Returncode (*farr_Values[38])(void);\n"}) )
  CHECK(633, String_concat(expected, &(String){32, 31, "  Array farr_Var = {38, NULL};\n"}) )
  CHECK(634, String_concat(expected, &(String){28, 27, "  Array* farr = &farr_Var;\n"}) )
  CHECK(635, String_concat(expected, &(String){34, 33, "  farr_Var.values = farr_Values;\n"}) )
  CHECK(636, String_concat(expected, &(String){50, 49, "  if ((3) < 0 || (3) >= (farr)->length) RAISE(2)\n"}) )
  CHECK(637, String_concat(expected, &(String){47, 46, "  ((Returncode (**)(void))((farr)->values))[3]"}) )
  CHECK(638, String_concat(expected, &(String){10, 9, " = fun0;\n"}) )
  CHECK(639, String_concat(expected, &(String){50, 49, "  if ((3) < 0 || (3) >= (farr)->length) RAISE(3)\n"}) )
  CHECK(640, String_concat(expected, &(String){51, 50, "  if (((Returncode (**)(void))((farr)->values))[3]"}) )
  CHECK(641, String_concat(expected, &(String){20, 19, " == NULL) RAISE(3)\n"}) )
  CHECK(642, String_concat(expected, &(String){10, 9, "  CHECK(3"}) )
  CHECK(643, String_concat(expected, &(String){53, 52, ", (((Returncode (**)(void))((farr)->values))[3])() )"}) )
  CHECK(644, test_code(&(String){58, 57, "var Array{38:Func{()}} farr\n  farr[3] := fun0\n  farr[3]()"}, expected) )
  CHECK(647, test_code_error(&(String){15, 14, "var Func error"}, &(String){35, 34, "missing arguments in function type"}) )
  CHECK(649, test_code_error(&(String){24, 23, "var Array{3:Func} error"}, &(String){35, 34, "missing arguments in function type"}) )
  CHECK(651, test_code_error(&(String){17, 16, "var Func{} error"}, &(String){43, 42, "expected \"(\" inside Function type, got \"}\""}) )
  CHECK(654, test_code_error(&(String){20, 19, "var Func{() } error"}, &(String){22, 21, "expected \"}\", got \" \""}) )
  CHECK(655, test_code_error(&(String){23, 22, "var Func{()} fun(fun1)"}, &(String){20, 19, "too many parameters"}) )
  CHECK(657, test_code_error(&(String){45, 44, "var Func{()->(copy Int x)} fun\n  fun := fun0"}, &(String){16, 15, "too few outputs"}) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
...
Returncode test_builtin() {
  CHECK(663, test_code(&(String){16, 15, "i.str(user str)"}, &(String){27, 26, "CHECK(1, Int_str(i, str) )"}) )
  CHECK(664, test_code(&(String){10, 9, "b := true"}, &(String){10, 9, "b = true;"}) )
  CHECK(665, test_code(&(String){11, 10, "b := false"}, &(String){11, 10, "b = false;"}) )
  CHECK(666, test_code(&(String){9, 8, "c := EOF"}, &(String){9, 8, "c = EOF;"}) )
  CHECK(667, test_code(&(String){16, 15, "i := arr.length"}, &(String){45, 44, "if (arr == NULL) RAISE(1)\n  i = arr->length;"}) )
  CHECK(670, test_code(&(String){16, 15, "i := str.length"}, &(String){45, 44, "if (str == NULL) RAISE(1)\n  i = str->length;"}) )
  CHECK(673, test_code(&(String){12, 11, "str.clear()"}, &(String){29, 28, "CHECK(1, String_clear(str) )"}) )
  CHECK(674, test_code(&(String){30, 29, "str.equal(user str)->(copy b)"}, &(String){40, 39, "CHECK(1, String_equal(str, str, &(b)) )"}) )
  CHECK(677, test_code(&(String){26, 25, "str.get(copy i)->(copy c)"}, &(String){36, 35, "CHECK(1, String_get(str, i, &(c)) )"}) )
  CHECK(680, test_code(&(String){19, 18, "str.append(copy c)"}, &(String){33, 32, "CHECK(1, String_append(str, c) )"}) )
  CHECK(681, test_code(&(String){19, 18, "str.copy(user str)"}, &(String){33, 32, "CHECK(1, String_copy(str, str) )"}) )
  CHECK(682, test_code(&(String){21, 20, "str.concat(user str)"}, &(String){35, 34, "CHECK(1, String_concat(str, str) )"}) )
  CHECK(684, test_code(&(String){23, 22, "str.concat-int(copy i)"}, &(String){37, 36, "CHECK(1, String_concat_int(str, i) )"}) )
  CHECK(687, test_code(&(String){29, 28, "str.find(user str)->(copy i)"}, &(String){39, 38, "CHECK(1, String_find(str, str, &(i)) )"}) )
  CHECK(690, test_code(&(String){26, 25, "str.has(copy c)->(copy b)"}, &(String){36, 35, "CHECK(1, String_has(str, c, &(b)) )"}) )
  CHECK(693, test_code(&(String){39, 38, "file-open-read(user str)->(owner fobj)"}, &(String){40, 39, "CHECK(1, file_open_read(str, &(fobj)) )"}) )
  CHECK(696, test_code(&(String){40, 39, "file-open-write(user str)->(owner fobj)"}, &(String){41, 40, "CHECK(1, file_open_write(str, &(fobj)) )"}) )
  CHECK(699, test_code(&(String){13, 12, "fobj.close()"}, &(String){28, 27, "CHECK(1, File_close(fobj) )"}) )
  CHECK(700, test_code(&(String){22, 21, "fobj.getc()->(copy c)"}, &(String){33, 32, "CHECK(1, File_getc(fobj, &(c)) )"}) )
  CHECK(702, test_code(&(String){18, 17, "fobj.putc(copy c)"}, &(String){30, 29, "CHECK(1, File_putc(fobj, c) )"}) )
  CHECK(703, test_code(&(String){21, 20, "fobj.write(user str)"}, &(String){33, 32, "CHECK(1, File_write(fobj, str) )"}) )
  CHECK(705, test_code(&(String){34, 33, "user Array{String} argv(sys.argv)"}, &(String){68, 67, "Array* argv = NULL;\n  if (sys == NULL) RAISE(1)\n  argv = sys->argv;"}) )
  CHECK(708, test_code(&(String){20, 19, "sys.print(user str)"}, &(String){31, 30, "CHECK(1, Sys_print(sys, str) )"}) )
  CHECK(709, test_code(&(String){22, 21, "sys.println(user str)"}, &(String){33, 32, "CHECK(1, Sys_println(sys, str) )"}) )
  CHECK(711, test_code(&(String){17, 16, "sys.exit(copy i)"}, &(String){28, 27, "CHECK(1, Sys_exit(sys, i) )"}) )
  CHECK(712, test_code(&(String){31, 30, "sys.system(user str)->(copy i)"}, &(String){38, 37, "CHECK(1, Sys_system(sys, str, &(i)) )"}) )
  CHECK(715, test_code(&(String){41, 40, "sys.getenv(user str, user str)->(copy b)"}, &(String){43, 42, "CHECK(1, Sys_getenv(sys, str, str, &(b)) )"}) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
