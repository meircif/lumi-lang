...
Returncode f_assert_string_slice(String* expected, String* actual, Int start, Int slice_length, Bool is_main) {
  ...
  Bool _Bool145;
  CHECK(101, String_equal(actual_slice, expected, &(_Bool145)) )
  if (_Bool145) {
    return OK;
  }
...
Returncode set_test_file_name(String* filename, String* base_name, String* suffix) {
  ...
  Bool _Bool146;
  CHECK(127, Sys_getenv(sys, &(String){9, 8, "TEST_DIR"}, base_dir, &(_Bool146)) )
  if (_Bool146) {
...
Returncode test_new_file(String* base_name, String* name) {
  ...
  Bool _Bool147;
  CHECK(144, read_line(input_file, line, &(String){7, 6, "## @@ "}, &(_Bool147)) )
  if (!_Bool147) {
    CHECK(145, file_error(name, line) )
  }
  Bool _Bool148;
  CHECK(146, read_line(expected_output_file, line, &(String){8, 7, "/// @@ "}, &(_Bool148)) )
  if (!_Bool148) {
...
Returncode test_from_file(String* name) {
  ...
  if (had_test_error) {
    while (true) {
      Bool _Bool149;
      CHECK(156, read_line(input_file, line, &(String){7, 6, "## @@ "}, &(_Bool149)) )
      if (!(!_Bool149)) break;
    }
    while (true) {
      Bool _Bool150;
      CHECK(158, read_line(expected_output_file, line, &(String){8, 7, "/// @@ "}, &(_Bool150)) )
      if (!(!_Bool150)) break;
    }
  }
  had_test_error = true;
  Bool _Bool151;
  CHECK(160, read_line(input_file, line, NULL, &(_Bool151)) )
  Bool _Bool152;
  CHECK(161, String_equal(line, name, &(_Bool152)) )
  if (!_Bool152) {
    CHECK(162, file_error(name, line) )
  }
  Bool _Bool153;
  CHECK(163, read_line(expected_output_file, line, NULL, &(_Bool153)) )
  Bool _Bool154;
  CHECK(164, String_equal(line, name, &(_Bool154)) )
  if (!_Bool154) {
  ...
  Bool _Bool155;
  CHECK(172, read_line(input_file, line, &(String){6, 5, "## @ "}, &(_Bool155)) )
  if (!_Bool155) {
    CHECK(173, file_error(name, line) )
  }
  Bool _Bool156;
  CHECK(174, read_line(expected_output_file, line, &(String){7, 6, "/// @ "}, &(_Bool156)) )
  if (!_Bool156) {
  ...
  while (true) {
    ...
    Char _Char157;
    CHECK(187, File_getc(input_file, &(_Char157)) )
    if (_Char157 != ' ') {
      CHECK(188, file_error(name, subname) )
    }
    Bool _Bool158;
    CHECK(189, read_line(input_file, subname, NULL, &(_Bool158)) )
    Bool _Bool159;
    CHECK(190, read_line(expected_output_file, line, NULL, &(_Bool159)) )
    Bool _Bool160;
    CHECK(191, String_equal(line, subname, &(_Bool160)) )
    if (!_Bool160) {
    ...
    Char _Char161;
    CHECK(210, File_getc(expected_output_file, &(_Char161)) )
    Char _Char162;
    CHECK(211, File_getc(input_file, &(_Char162)) )
    if (!(_Char162 == ' ')) break;
  }
  
  Char _Char163;
  CHECK(213, File_getc(expected_output_file, &(_Char163)) )
  Char _Char164;
  CHECK(214, File_getc(input_file, &(_Char164)) )
  if (_Char164 == EOF) {
    Char _Char165;
    CHECK(215, File_getc(expected_output_file, &(_Char165)) )
    if (_Char165 != EOF) {
...
Returncode read_line(File* file, String* line, String* prefix, Bool* found_prefix) {
  ...
  while (true) {
    ...
    if (NULL != prefix &&  ! (*found_prefix)) {
      Bool _Bool166;
      CHECK(235, String_equal(line, prefix, &(_Bool166)) )
      if (_Bool166) {
        (*found_prefix) = true;
        return OK;
      }
...
Returncode read_data(File* file, String* data, String* prefix) {
  ...
  while (true) {
    Bool _Bool167;
    CHECK(246, read_line(file, line, prefix, &(_Bool167)) )
    if (!(!_Bool167)) break;
    if (data->length > 0) {
...
Returncode test_list() {
  ...
  String* _String168;
  CHECK(265, List_pop(list, (void**)&(_String168)) )
  TEST_ASSERT(265, !(NULL != _String168))
  String* _String169;
  CHECK(266, string_new_copy(&(String){7, 6, "value1"}, &(_String169)) )
  CHECK(266, List_add(list, _String169) )
  String* _String170;
  CHECK(267, string_new_copy(&(String){7, 6, "value2"}, &(_String170)) )
  CHECK(267, List_add(list, _String170) )
  String* _String171;
  CHECK(268, string_new_copy(&(String){7, 6, "value3"}, &(_String171)) )
  CHECK(268, List_add(list, _String171) )
  String* _String172;
  CHECK(269, List_pop(list, (void**)&(_String172)) )
  CHECK(269, f_assert_string(&(String){7, 6, "value1"}, _String172) )
  String* _String173;
  CHECK(270, List_pop(list, (void**)&(_String173)) )
  CHECK(270, f_assert_string(&(String){7, 6, "value2"}, _String173) )
  String* _String174;
  CHECK(271, List_pop(list, (void**)&(_String174)) )
  CHECK(271, f_assert_string(&(String){7, 6, "value3"}, _String174) )
  String* _String175;
  CHECK(272, List_pop(list, (void**)&(_String175)) )
  TEST_ASSERT(272, !(NULL != _String175))
  return OK;
}
#undef MR_FUNC_NAME
#endif
...
Returncode test_name_map() {
  ...
  String* _String176;
  CHECK(278, NameMap_find(map, &(String){6, 5, "name1"}, (void**)&(_String176)) )
  TEST_ASSERT(278, !(NULL != _String176))
  CHECK(279, NameMap_add(map, &(String){6, 5, "name1"}, &(String){7, 6, "value1"}) )
  String* _String177;
  CHECK(280, NameMap_find(map, &(String){6, 5, "name1"}, (void**)&(_String177)) )
  CHECK(280, f_assert_string(&(String){7, 6, "value1"}, _String177) )
  String* _String178;
  CHECK(281, NameMap_find(map, &(String){6, 5, "name2"}, (void**)&(_String178)) )
  TEST_ASSERT(281, !(NULL != _String178))
  CHECK(282, NameMap_add(map, &(String){6, 5, "name2"}, &(String){7, 6, "value2"}) )
  String* _String179;
  CHECK(283, NameMap_find(map, &(String){6, 5, "name1"}, (void**)&(_String179)) )
  CHECK(283, f_assert_string(&(String){7, 6, "value1"}, _String179) )
  String* _String180;
  CHECK(284, NameMap_find(map, &(String){6, 5, "name2"}, (void**)&(_String180)) )
  CHECK(284, f_assert_string(&(String){7, 6, "value2"}, _String180) )
  CHECK(285, NameMap_update_or_add(map, &(String){6, 5, "name3"}, &(String){7, 6, "value3"}) )
  String* _String181;
  CHECK(286, NameMap_find(map, &(String){6, 5, "name3"}, (void**)&(_String181)) )
  CHECK(286, f_assert_string(&(String){7, 6, "value3"}, _String181) )
  CHECK(287, NameMap_update_or_add(map, &(String){6, 5, "name2"}, &(String){8, 7, "value2x"}) )
  String* _String182;
  CHECK(288, NameMap_find(map, &(String){6, 5, "name2"}, (void**)&(_String182)) )
  CHECK(288, f_assert_string(&(String){8, 7, "value2x"}, _String182) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
...
#define RETURN_ERROR(value) break
    DeclarationArgument* _DeclarationArgument183;
    CHECK(294, (arg)->_base._dtl[4](arg, &(_DeclarationArgument183)) );
#undef RETURN_ERROR
...
#define RETURN_ERROR(value) break
    Char _Char184;
    CHECK(295, (arg)->_base._dtl[5](arg, NULL, &(_Char184)) );
#undef RETURN_ERROR
#define RETURN_ERROR(value) return value
    _trace_stream = stdout;
    TEST_FAIL(295)
...
#undef RETURN_ERROR
#define RETURN_ERROR(value) break
    TypeInstance* _TypeInstance185;
    CHECK(296, (arg)->_base._dtl[6](arg, &(_TypeInstance185)) );
#undef RETURN_ERROR
#define RETURN_ERROR(value) return value
    _trace_stream = stdout;
    TEST_FAIL(296)
...
#undef RETURN_ERROR
#define RETURN_ERROR(value) break
    SyntaxTreeVariable* _SyntaxTreeVariable186;
    CHECK(297, (arg)->_base._dtl[8](arg, &(_SyntaxTreeVariable186)) );
#undef RETURN_ERROR
#define RETURN_ERROR(value) return value
    _trace_stream = stdout;
    TEST_FAIL(297)
...
#undef RETURN_ERROR
#define RETURN_ERROR(value) break
    Expression* _Expression187;
    CHECK(298, (arg)->_base._dtl[9](arg, &(_Expression187)) );
#undef RETURN_ERROR
#define RETURN_ERROR(value) return value
    _trace_stream = stdout;
    TEST_FAIL(298)
...
#undef RETURN_ERROR
#define RETURN_ERROR(value) break
    Argument* _Argument188;
    CHECK(302, (factory)->_dtl[0](factory, &(_Argument188)) );
#undef RETURN_ERROR
#define RETURN_ERROR(value) return value
    _trace_stream = stdout;
    TEST_FAIL(302)
...
#define RETURN_ERROR(value) break
    Char _Char189;
    CHECK(304, (branch)->_base._dtl[4](branch, NULL, &(_Char189)) );
#undef RETURN_ERROR
#define RETURN_ERROR(value) return value
    _trace_stream = stdout;
    TEST_FAIL(304)
