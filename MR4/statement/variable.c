...
Returncode SyntaxTreeVariable_check_name(SyntaxTreeVariable* self) {
  Bool _Bool178;
  CHECK(64, f_is_legal_name(self->name, NAME_DEFAULT, &(_Bool178)) )
  if (!_Bool178) {
  ...
  if (NULL != self->parent_type) {
    ...
    Int _Int179;
    CHECK(68, TypeData_find_field(self->parent_type, self->name, &(field), &(_Int179)) )
    if (NULL != field) {
    ...
    Int _Int180;
    CHECK(72, TypeData_find_meth(self->parent_type, self->name, &(meth), &(_Int180)) )
    if (NULL != meth) {
      CHECK(74, SyntaxTreeNode_m_syntax_error(&(self->_base._base), &(String){28, 27, "field name overrides method"}, self->name) )
    }
  }
  else {
    ModuleMembers* _ModuleMembers181;
    CHECK(77, NameMap_find(glob->module_map, self->name, (void**)&(_ModuleMembers181)) )
    if (NULL != _ModuleMembers181) {
      CHECK(78, SyntaxTreeNode_m_syntax_error(&(self->_base._base), &(String){31, 30, "variable name overrides module"}, self->name) )
    }
    SyntaxTreeVariable* _SyntaxTreeVariable182;
    CHECK(80, Global_find_variable(glob, self->name, &(_SyntaxTreeVariable182)) )
    if (NULL != _SyntaxTreeVariable182) {
      CHECK(81, SyntaxTreeNode_m_syntax_error(&(self->_base._base), &(String){32, 31, "redefinition of global variable"}, self->name) )
    }
    SyntaxTreeFunction* _SyntaxTreeFunction183;
    CHECK(83, Global_find_function(glob, self->name, &(_SyntaxTreeFunction183)) )
    if (NULL != _SyntaxTreeFunction183) {
    ...
    if (NULL != self->_base.parent) {
      SyntaxTreeVariable* _SyntaxTreeVariable184;
      CHECK(87, (self->_base.parent)->_base._base._dtl[7](self->_base.parent, self->name, NULL, &(_SyntaxTreeVariable184)) )
      if (NULL != _SyntaxTreeVariable184) {
...
Returncode SyntaxTreeVariable_find_variable(SyntaxTreeVariable* self, String* name, String* module_name, SyntaxTreeVariable** variable) {
  Bool _Bool185;
  CHECK(101, String_equal(self->name, name, &(_Bool185)) )
  if (_Bool185) {
    (*variable) = self;
  }
  else {
...
#define MR_FUNC_NAME _func_name_SyntaxTreeVariable_write_cname
Returncode SyntaxTreeVariable_write_cname(SyntaxTreeVariable* self) {
  if (NULL != self->my_module) {
...
Returncode SyntaxTreeVariable_write_spaces(SyntaxTreeVariable* self) {
  if (NULL != self->_base.parent) {
    SyntaxTreeFunction* _SyntaxTreeFunction186;
    CHECK(315, (self->_base.parent)->_base._base._dtl[8](self->_base.parent, &(_SyntaxTreeFunction186)) )
    CHECK(315, SyntaxTreeBranch_write_spaces(&(_SyntaxTreeFunction186->_base._base)) )
  }
  else {
    if (NULL != self->parent_type) {
...
Returncode VariableInit_parse(VariableInit* self, SyntaxTreeVariable* variable, Char* end) {
  ...
  *self->expression_init = (InitExpression){InitExpression__dtl, NULL, 0, NULL, NULL, 0, false, false, false, false, false, false, NULL, NULL, NULL, NULL, NULL};
  ...
  TypeInstance* _TypeInstance187;
  CHECK(333, TypeInstance_copy_new(variable->type_instance, &(_TypeInstance187)) )
  CHECK(333, InitExpression_parse(self->expression_init, _TypeInstance187, NULL, &(self->_base), &((*end))) )
...
Returncode VariableInit_write(VariableInit* self) {
  ...
  if (self->_base.parent == &(glob->root->global_init->_base._base)) {
    CHECK(357, write(&(String){23, 22, "#define MR_FILE_NAME \""}) )
  ...
  if (self->_base.parent == &(glob->root->global_init->_base._base)) {
    CHECK(362, write(&(String){21, 20, "#undef MR_FILE_NAME\n"}) )
  }
  return OK;
}
#undef MR_FUNC_NAME
...
Returncode SyntaxTreeConstant_parse(SyntaxTreeConstant* self, Char* end) {
  ...
  Int _Int188;
  CHECK(380, read_until(&(String){2, 1, " "}, false, &(type_name), &((*end)), &(_Int188)) )
  Bool _Bool189;
  CHECK(381, String_equal(type_name, &(String){4, 3, "Int"}, &(_Bool189)) )
  if (!_Bool189) {
  ...
  Bool _Bool190;
  CHECK(388, f_is_legal_name(self->_base.name, NAME_CONSTANT, &(_Bool190)) )
  if (!_Bool190) {
