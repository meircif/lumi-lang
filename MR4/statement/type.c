...
Returncode TypeData_parse_new(TypeData* self, Bool is_dynamic, Char* end, TypeData** new_node) {
  ...
  *(*new_node) = (TypeData){TypeData__dtl, NULL, 0, 0, NULL, NULL, NULL, NULL, NULL, NULL, NULL, false, false, false, false};
...
Returncode TypeData_parse(TypeData* self, Bool is_dynamic, Char* end) {
  ...
  if ((*end) == '{') {
    ...
    if (self->parameters == NULL) RAISE(26)
    *self->parameters = (List){NULL, NULL};
    while (true) {
      String* name = NULL;
      CHECK(29, read_new(&(String){3, 2, ":}"}, &(name), &((*end))) )
      Bool _Bool137;
      CHECK(30, f_is_legal_name(name, true, &(_Bool137)) )
      if (!_Bool137) {
        CHECK(31, SyntaxTreeNode_m_syntax_error(&(self->_base._base._base), &(String){28, 27, "illegal type parameter name"}, name) )
      }
      CHECK(32, List_add(self->parameters, name) )
      if (!((*end) == ':')) break;
    }
    if ((*end) != '}') {
      CHECK(35, SyntaxTreeNode_m_syntax_error_c(&(self->_base._base._base), &(String){40, 39, "expected \"}\" after type parameters, got"}, (*end)) )
    }
    CHECK(37, read_c(&((*end))) )
  }
  if ((*end) == '(') {
    self->base_type = malloc(sizeof(TypeInstance));
    if (self->base_type == NULL) RAISE(39)
    *self->base_type = (TypeInstance){NULL, NULL, NULL, NULL, NULL, NULL};
    CHECK(40, TypeInstance_parse(self->base_type, &(String){2, 1, ")"}, &(self->_base._base._base), NULL, &((*end))) )
    if ((*end) != ')') {
      CHECK(42, SyntaxTreeNode_m_syntax_error_c(&(self->_base._base._base), &(String){34, 33, "expected \")\" after base type, got"}, (*end)) )
    }
    CHECK(44, read_c(&((*end))) )
  }
  self->_base._base.indentation_spaces = 2;
  CHECK(46, SyntaxTreeBranch_parse_block_children(&(self->_base._base), self, NULL, &((*end))) )
...
Returncode TypeData_add_type(TypeData* self) {
  Bool _Bool138;
  CHECK(50, f_is_legal_name(self->name, true, &(_Bool138)) )
  if (!_Bool138) {
    CHECK(51, SyntaxTreeNode_m_syntax_error(&(self->_base._base._base), &(String){18, 17, "illegal type name"}, self->name) )
  }
  TypeData* _TypeData139;
  CHECK(52, NameMap_find(glob->type_map, self->name, (void**)&(_TypeData139)) )
  if (NULL != _TypeData139) {
    CHECK(53, SyntaxTreeNode_m_syntax_error(&(self->_base._base._base), &(String){21, 20, "redefinition of type"}, self->name) )
  }
  CHECK(54, Global_add_type(glob, self) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
...
Returncode TypeData_parse_child(TypeData* self, String* keyword, Char* end) {
  Bool _Bool140;
  CHECK(57, SyntaxTreeNamespace_parse_if_function(&(self->_base), keyword, self, &((*end)), &(_Bool140)) )
  if (_Bool140) {
    return OK;
  }
  
  Bool _Bool141;
  CHECK(60, String_equal(keyword, &(String){4, 3, "new"}, &(_Bool141)) )
  if (_Bool141) {
    if ((*end) != '(') {
      CHECK(62, SyntaxTreeNode_m_syntax_error_c(&(self->_base._base._base), &(String){30, 29, "expected \"(\" after \"new\", got"}, (*end)) )
    }
    if (NULL != self->constructor) {
      CHECK(65, SyntaxTreeNode_m_syntax_error(&(self->_base._base._base), &(String){37, 36, "constructor already defined for type"}, self->name) )
    }
    SyntaxTreeConstructor* constructor = NULL;
    CHECK(68, SyntaxTreeConstructor_parse_new(NULL, self, &((*end)), &(constructor)) )
    self->constructor = constructor;
    CHECK(70, List_add(self->_base.functions, &(constructor->_base)) )
    
  }
  else {
    CHECK(73, SyntaxTreeNode_m_syntax_error(&(self->_base._base._base), &(String){16, 15, "unknown keyword"}, keyword) )
  }
  return OK;
}
#undef MR_FUNC_NAME
...
Returncode TypeData_m_new_type_instance(TypeData* self, TypeInstance** type_instance) {
  ...
  if ((*type_instance) == NULL) RAISE(81)
...
Returncode TypeData_m_self_type_instance(TypeData* self, TypeInstance** type_instance) {
  CHECK(85, TypeData_m_new_type_instance(self, &((*type_instance))) )
  if (NULL != self->parameters) {
    (*type_instance)->parameters = malloc(sizeof(List));
    if ((*type_instance)->parameters == NULL) RAISE(87)
    *(*type_instance)->parameters = (List){NULL, NULL};
    ...
    while (true) {
      ...
      if (parameter == NULL) RAISE(91)
      *parameter = (TypeInstance){NULL, NULL, NULL, NULL, NULL, NULL};
      parameter->type_data = &(glob->type_generic->_base);
      CHECK(93, string_new_copy(node->item, &(parameter->name)) )
      CHECK(94, List_add((*type_instance)->parameters, parameter) )
      node = node->next;
    }
  }
...
Returncode TypeData_find_field(TypeData* self, String* name, SyntaxTreeVariable** field, Int* bases) {
  ...
  while (true) {
    ...
    while (true) {
      if (!(NULL != child)) break;
      Bool _Bool142;
      CHECK(110, String_equal(((SyntaxTreeVariable*)(child->item))->name, name, &(_Bool142)) )
      if (_Bool142) {
...
Returncode TypeData_find_meth(TypeData* self, String* name, SyntaxTreeFunction** method, Int* bases) {
  ...
  while (true) {
    ...
    while (true) {
      if (!(NULL != child)) break;
      Bool _Bool143;
      CHECK(128, String_equal(((SyntaxTreeFunction*)(child->item))->name, name, &(_Bool143)) )
      if (_Bool143) {
...
Returncode TypeData_m_order_bases(TypeData* self, List* ordered_list) {
  ...
  if (NULL != self->base_type) {
    CHECK(141, SyntaxTreeNode_find_type(&(self->_base._base._base), self->base_type->name, &(self->base_type->type_data)) )
    CHECK(142, TypeData_m_order_bases(self->base_type->type_data, ordered_list) )
  }
  CHECK(143, List_add(ordered_list, self) )
  /* init `self.dynamic-base-methods` and test override methods */
  if (self->is_dynamic) {
    self->dynamic_base_methods = malloc(sizeof(NameMap));
    if (self->dynamic_base_methods == NULL) RAISE(146)
  ...
  while (true) {
    ...
    Int bases = 0;
    if (NULL != self->base_type) {
      CHECK(153, TypeData_find_meth(self->base_type->type_data, ((SyntaxTreeFunction*)(child->item))->name, &(method), &(bases)) )
    }
    if (NULL != method) {
      CHECK(156, SyntaxTreeFunction_m_compare(((SyntaxTreeFunction*)(child->item)), method) )
      ...
      if (((SyntaxTreeFunction*)(child->item))->is_dynamic) {
        ...
        CHECK(162, NameMap_add(self->dynamic_base_methods, ((SyntaxTreeFunction*)(child->item))->name, child->item) )
      }
    }
    child = child->next;
  }
...
Returncode TypeData_link_types(TypeData* self) {
  if (NULL != self->base_type) {
    CHECK(168, TypeInstance_link_types(self->base_type, &(self->_base._base._base)) )
  }
  CHECK(169, SyntaxTreeNamespace_link_types(&(self->_base)) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
...
Returncode TypeData_analyze(TypeData* self) {
  if (NULL != self->base_type) {
    CHECK(173, TypeInstance_analyze_lengths(self->base_type, &(self->_base._base._base)) )
  }
  Bool _Bool144;
  CHECK(174, List_m_is_empty(self->_base._base.variables, &(_Bool144)) )
  if (!(NULL != self->base_type) && _Bool144) {
    CHECK(175, SyntaxTreeNode_m_syntax_error(&(self->_base._base._base), &(String){20, 19, "type with no fields"}, self->name) )
  }
  if (self->is_dynamic &&  ! (NULL != self->base_type && self->base_type->type_data->is_dynamic)) {
    ListNode* node = self->_base.functions->first;
    while (true) {
      if (!(NULL != node)) {
        CHECK(181, SyntaxTreeNode_m_syntax_error(&(self->_base._base._base), &(String){30, 29, "class with no dynamic methods"}, self->name) )
      }
      if (!(!((SyntaxTreeFunction*)(node->item))->is_dynamic)) break;
      node = node->next;
    }
  }
  CHECK(185, SyntaxTreeNamespace_analyze(&(self->_base)) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
...
Returncode TypeData_write_declaration(TypeData* self) {
  ...
  CHECK(190, write(&(String){17, 16, "\ntypedef struct "}) )
  CHECK(191, write_cname(self->name) )
  CHECK(192, write(&(String){2, 1, " "}) )
  CHECK(193, write_cname(self->name) )
  CHECK(194, write(&(String){3, 2, ";\n"}) )
  if (self->is_dynamic) {
    CHECK(196, write(&(String){17, 16, "\ntypedef struct "}) )
    CHECK(197, write_cname(self->name) )
    CHECK(198, write(&(String){10, 9, "_Dynamic "}) )
...
Returncode TypeData_write(TypeData* self) {
  ...
  /* }; */
  CHECK(209, write(&(String){9, 8, "\nstruct "}) )
  CHECK(210, write_cname(self->name) )
  CHECK(211, write(&(String){4, 3, " {\n"}) )
  self->_base._base.indentation_spaces = 2;
  if (NULL != self->base_type) {
    CHECK(214, SyntaxTreeBranch_write_spaces(&(self->_base._base)) )
    CHECK(215, write_cname(self->base_type->type_data->name) )
    CHECK(216, write(&(String){9, 8, " _base;\n"}) )
  }
  CHECK(217, SyntaxTreeNode_write_children(&(self->_base._base._base), self->_base._base.variables) )
  CHECK(218, write(&(String){4, 3, "};\n"}) )
  if (self->is_dynamic) {
    CHECK(220, write(&(String){9, 8, "\nstruct "}) )
    CHECK(221, write_cname(self->name) )
    CHECK(222, write(&(String){12, 11, "_Dynamic {\n"}) )
    if (NULL != self->base_type && self->base_type->type_data->is_dynamic) {
      CHECK(224, SyntaxTreeBranch_write_spaces(&(self->_base._base)) )
      CHECK(225, write_cname(self->base_type->type_data->name) )
      CHECK(226, write(&(String){17, 16, "_Dynamic _base;\n"}) )
    }
    else {
      CHECK(228, SyntaxTreeBranch_write_spaces(&(self->_base._base)) )
      CHECK(229, write(&(String){19, 18, "Dynamic_Del _del;\n"}) )
    }
    NameMapNode* child = self->dynamic_base_methods->first;
    while (true) {
      if (!(NULL != child)) break;
      CHECK(233, SyntaxTreeBranch_write_spaces(&(self->_base._base)) )
      CHECK(234, SyntaxTreeFunction_write_pointer(((SyntaxTreeFunction*)(child->value))) )
...
Returncode TypeData_write_methods_declaration(TypeData* self) {
  CHECK(241, SyntaxTreeNamespace_write_functions_declaration(&(self->_base)) )
  /* void `cname`_Del(`cname`* self); */
  CHECK(243, write(&(String){7, 6, "\nvoid "}) )
  CHECK(244, write_cname(self->name) )
  CHECK(245, write(&(String){6, 5, "_Del("}) )
  CHECK(246, write_cname(self->name) )
  CHECK(247, write(&(String){10, 9, "* self);\n"}) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
...
Returncode TypeData_write_global(TypeData* self) {
  ...
  if (self->is_dynamic) {
    ...
    CHECK(253, write_cname(self->name) )
    CHECK(254, write(&(String){10, 9, "_Dynamic "}) )
    CHECK(255, write_cname(self->name) )
    CHECK(256, write(&(String){12, 11, "_dynamic = "}) )
    CHECK(257, TypeData_write_dynamic_init(self, self) )
  }
  else {
    /* Generic_Type_Dynamic `name`_dynamic = {(Dynamic_Del)`name`_Del}; */
    CHECK(260, write(&(String){22, 21, "Generic_Type_Dynamic "}) )
    CHECK(261, write_cname(self->name) )
    CHECK(262, write(&(String){26, 25, "_dynamic = {(Dynamic_Del)"}) )
    CHECK(263, write_cname(self->name) )
    CHECK(264, write(&(String){6, 5, "_Del}"}) )
  }
...
Returncode TypeData_write_dynamic_init(TypeData* self, TypeData* type_data) {
  ...
  if (NULL != type_data->base_type && type_data->base_type->type_data->is_dynamic) {
    CHECK(270, TypeData_write_dynamic_init(self, type_data->base_type->type_data) )
  }
  else {
    CHECK(272, write(&(String){14, 13, "(Dynamic_Del)"}) )
    CHECK(273, write_cname(self->name) )
    CHECK(274, write(&(String){5, 4, "_Del"}) )
  }
  ...
  while (true) {
    ...
    SyntaxTreeFunction* method = NULL;
    Int _Int145;
    CHECK(279, TypeData_find_meth(self, ((SyntaxTreeFunction*)(child->value))->name, &(method), &(_Int145)) )
    CHECK(280, write(&(String){3, 2, ", "}) )
    if (method != ((SyntaxTreeFunction*)(child->value))) {
      CHECK(282, write(&(String){7, 6, "(Func)"}) )
    }
    CHECK(283, write_cname(method->parent_type->name) )
    CHECK(284, write(&(String){2, 1, "_"}) )
    CHECK(285, write_cname(method->name) )
    child = child->next;
  }
  CHECK(287, write(&(String){2, 1, "}"}) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
...
Returncode TypeData_write_methods_body(TypeData* self) {
  CHECK(290, SyntaxTreeNode_write_children(&(self->_base._base._base), self->_base.functions) )
  /* void `cname`_Del(`cname`* self) { */
  /*   if (self == NULL) return; */
  /*   `base`_Del(self); */
  /*   ... */
  /* } */
  CHECK(296, write(&(String){7, 6, "\nvoid "}) )
  CHECK(297, write_cname(self->name) )
  CHECK(298, write(&(String){6, 5, "_Del("}) )
  CHECK(299, write_cname(self->name) )
  CHECK(300, write(&(String){11, 10, "* self) {\n"}) )
  CHECK(301, write(&(String){29, 28, "  if (self == NULL) return;\n"}) )
  if (NULL != self->base_type) {
    CHECK(303, write(&(String){3, 2, "  "}) )
    CHECK(304, write_cname(self->base_type->type_data->name) )
    CHECK(305, write(&(String){23, 22, "_Del(&(self->_base));\n"}) )
  }
  if (self->is_delete_mocked) {
    CHECK(307, write(&(String){18, 17, "  IGNORE_ERRORS( "}) )
    CHECK(308, write_cname(self->name) )
    CHECK(309, write(&(String){18, 17, "_MockDel(self) )\n"}) )
  }
  CHECK(310, SyntaxTreeBranch_write_cleanup(&(self->_base._base), self) )
...
Returncode TypeData_write_me(TypeData* self, TypeWriter* type_writer) {
  CHECK(314, (type_writer)->_dtl[0](type_writer, self) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
...
Returncode TypeDeclarationWriter_write(TypeDeclarationWriter* self, TypeData* type_data) {
  CHECK(323, (type_data)->_base._base._base._dtl[6](type_data) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
...
Returncode TypeMethodsDeclarationWriter_write(TypeMethodsDeclarationWriter* self, TypeData* type_data) {
  CHECK(327, (type_data)->_base._base._base._dtl[7](type_data) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
...
Returncode TypeGlobalWriter_write(TypeGlobalWriter* self, TypeData* type_data) {
  CHECK(331, (type_data)->_base._base._base._dtl[8](type_data) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
...
Returncode TypeMethodsBodyWriter_write(TypeMethodsBodyWriter* self, TypeData* type_data) {
  CHECK(335, (type_data)->_base._base._base._dtl[9](type_data) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
