  ...
  if ((*end) == '{') {
    ...
    *self->parameters = (List){NULL, NULL};
    while (true) {
      ...
      Bool _Bool168;
      CHECK(33, f_is_legal_name(name, NAME_TYPE, &(_Bool168)) )
      if (!_Bool168) {
...
Returncode TypeData_add_type(TypeData* self, ModuleMembers* my_module) {
  Bool _Bool169;
  CHECK(53, f_is_legal_name(self->name, NAME_TYPE, &(_Bool169)) )
  if (!_Bool169) {
    CHECK(54, SyntaxTreeNode_m_syntax_error(&(self->_base._base._base), &(String){18, 17, "illegal type name"}, self->name) )
  }
  TypeData* _TypeData170;
  CHECK(55, Global_find_type(glob, self->name, &(_TypeData170)) )
  if (NULL != _TypeData170) {
    CHECK(56, SyntaxTreeNode_m_syntax_error(&(self->_base._base._base), &(String){21, 20, "redefinition of type"}, self->name) )
  }
  EnumData* _EnumData171;
  CHECK(57, NameMap_find(glob->current_module->enum_map, self->name, (void**)&(_EnumData171)) )
  if (NULL != _EnumData171) {
...
Returncode TypeData_parse_child(TypeData* self, String* keyword, Char* end) {
  Bool _Bool172;
  CHECK(63, SyntaxTreeNamespace_parse_if_function(&(self->_base), keyword, self, &((*end)), &(_Bool172)) )
  if (_Bool172) {
    return OK;
  }
  
  Bool _Bool173;
  CHECK(66, String_equal(keyword, &(String){4, 3, "new"}, &(_Bool173)) )
  if (_Bool173) {
    if ((*end) != '(') {
...
Returncode TypeData_find_field(TypeData* self, String* name, SyntaxTreeVariable** field, Int* bases) {
  ...
  while (true) {
    ...
    while (true) {
      if (!(NULL != child)) break;
      Bool _Bool174;
      CHECK(116, String_equal(((SyntaxTreeVariable*)(child->item))->name, name, &(_Bool174)) )
      if (_Bool174) {
...
Returncode TypeData_find_meth(TypeData* self, String* name, SyntaxTreeFunction** method, Int* bases) {
  ...
  while (true) {
    ...
    while (true) {
      if (!(NULL != child)) break;
      Bool _Bool175;
      CHECK(134, String_equal(((SyntaxTreeFunction*)(child->item))->name, name, &(_Bool175)) )
      if (_Bool175) {
...
Returncode TypeData_analyze(TypeData* self) {
  ...
  Bool _Bool176;
  CHECK(189, List_m_is_empty(self->_base._base.variables, &(_Bool176)) )
  if (!(NULL != self->base_type) && _Bool176) {
...
#define MR_FUNC_NAME _func_name_TypeData_write_cname
Returncode TypeData_write_cname(TypeData* self) {
...
Returncode TypeData_write_dynamic_init(TypeData* self, TypeData* type_data) {
  ...
  while (true) {
    ...
    SyntaxTreeFunction* method = NULL;
    Int _Int177;
    CHECK(300, TypeData_find_meth(self, ((SyntaxTreeFunction*)(child->value))->name, &(method), &(_Int177)) )
