...
Returncode SyntaxTreeMockFunction_parse(SyntaxTreeMockFunction* self, Char* end) {
  ...
  Bool _Bool147;
  CHECK(155, String_equal(self->mocked_name, &(String){7, 6, "delete"}, &(_Bool147)) )
  if (NULL != self->type_name && _Bool147) {
...
Returncode SyntaxTreeMockFunction_analyze(SyntaxTreeMockFunction* self) {
  if (NULL != self->type_name) {
    ...
    Bool _Bool148;
    CHECK(166, String_equal(self->mocked_name, &(String){7, 6, "delete"}, &(_Bool148)) )
    if (_Bool148) {
      ...
      Int _Int149;
      CHECK(173, TypeData_find_meth(self->_base.parent_type, self->mocked_name, &(self->mocked_function), &(_Int149)) )
    ...
    Bool _Bool150;
    CHECK(182, String_equal(self->mocked_name, &(String){4, 3, "new"}, &(_Bool150)) )
    if (_Bool150) {
      ...
      CHECK(190, (((Argument*)(self->_base.arguments->outputs->first->item)))->_base._dtl[6](((Argument*)(self->_base.arguments->outputs->first->item)), &(_TypeInstance151)) )
      if (_TypeInstance151->type_data != &(glob->type_bool->_base)) {
      ...
      Bool _Bool152;
      CHECK(195, String_equal(self->mocked_name, &(String){7, 6, "delete"}, &(_Bool152)) )
      if (_Bool152) {
  ...
  if (NULL != self->mocked_function) {
    ...
    Bool _Bool153;
    CHECK(211, FunctionArguments_check_same_as(self->_base.arguments, self->mocked_function->arguments, NULL, 0, &(_Bool153)) )
