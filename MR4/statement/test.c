...
Returncode SyntaxTreeMockFunction_analyze(SyntaxTreeMockFunction* self) {
  ...
  if (NULL != self->type_name) {
    ...
    Int _Int131;
    CHECK(114, TypeData_find_meth(self->_base.parent_type, self->mocked_name, &(mocked_func), &(_Int131)) )
    if (!(NULL != mocked_func)) {
    ...
    CHECK(121, TypeData_m_new_type_instance(self->_base.parent_type, &(_TypeInstance132)) )
    CHECK(121, FunctionArguments_add_self_parameter(self->_base.arguments, _TypeInstance132) )
  }
  else {
  ...
  Bool _Bool133;
  CHECK(128, FunctionArguments_check_same_as(self->_base.arguments, mocked_func->arguments, NULL, 0, &(_Bool133)) )
