...
Returncode SyntaxTreeVariable_parse_new(SyntaxTreeVariable* self, Int access, SyntaxTreeType* parent_type, SyntaxTreeVariable** new_node);
...
Returncode SyntaxTreeVariable_parse_new(SyntaxTreeVariable* self, Int access, SyntaxTreeType* parent_type, SyntaxTreeVariable** new_node) {
  ...
  *(*new_node) = (SyntaxTreeVariable){SyntaxTreeVariable__dtl, NULL, NULL, NULL, NULL};
  ...
  CHECK(12, SyntaxTreeVariable_parse((*new_node), access, parent_type) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
#if MR_STAGE == MR_DECLARATIONS
Returncode SyntaxTreeVariable_parse(SyntaxTreeVariable* self, Int access, SyntaxTreeType* parent_type);
#elif MR_STAGE == MR_FUNCTIONS
...
Returncode SyntaxTreeVariable_parse(SyntaxTreeVariable* self, Int access, SyntaxTreeType* parent_type) {
  ...
  *self->variable->type_instance = (TypeInstance){NULL, NULL};
  ...
  if (end != '\n') {
    CHECK(32, f_syntax_error_c(&(String){23, 22, "expected new-line, got"}, end) )
  }
  return OK;
}
#undef MR_FUNC_NAME
...
Returncode SyntaxTreeVariable_write(SyntaxTreeVariable* self) {
  CHECK(35, write(&(String){8, 7, "Access("}) )
  CHECK(36, write_int(self->variable->access) )
  CHECK(37, write(&(String){3, 2, ") "}) )
  CHECK(38, TypeInstance_write(self->variable->type_instance) )
  CHECK(39, write(&(String){2, 1, " "}) )
  CHECK(40, write(self->variable->name) )
  if (NULL != self->init_value) {
    CHECK(42, write(&(String){3, 2, " ("}) )
    CHECK(43, (self->init_value)->_dtl[0](self->init_value) )
