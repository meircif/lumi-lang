...
Returncode TypeData_parse(TypeData* self, Bool is_dynamic, Char* end) {
  ...
  Bool _Bool112;
  CHECK(23, f_is_legal_name(self->name, true, &(_Bool112)) )
  if (!_Bool112) {
    CHECK(24, SyntaxTreeNode_m_syntax_error(&(self->_base._base._base), &(String){18, 17, "illegal type name"}, self->name) )
  }
  TypeData* _TypeData113;
  CHECK(25, NameMap_find(glob->type_map, self->name, (void**)&(_TypeData113)) )
  if (NULL != _TypeData113) {
...
Returncode TypeData_parse_child(TypeData* self, String* keyword, Char* end) {
  Bool _Bool114;
  CHECK(39, SyntaxTreeNamespace_parse_if_function(&(self->_base), keyword, self, &((*end)), &(_Bool114)) )
  if (_Bool114) {
    return OK;
  }
  
  Bool _Bool115;
  CHECK(42, String_equal(keyword, &(String){4, 3, "new"}, &(_Bool115)) )
  if (_Bool115) {
    if ((*end) != '(') {
...
Returncode TypeData_find_field(TypeData* self, String* name, SyntaxTreeVariable** field, Int* bases) {
  ...
  while (true) {
    ...
    while (true) {
      if (!(NULL != child)) break;
      Bool _Bool116;
      CHECK(75, String_equal(((SyntaxTreeVariable*)(child->item))->name, name, &(_Bool116)) )
      if (_Bool116) {
...
Returncode TypeData_find_meth(TypeData* self, String* name, SyntaxTreeFunction** method, Int* bases) {
  ...
  while (true) {
    ...
    while (true) {
      if (!(NULL != child)) break;
      Bool _Bool117;
      CHECK(92, String_equal(((SyntaxTreeFunction*)(child->item))->name, name, &(_Bool117)) )
      if (_Bool117) {
...
Returncode TypeData_analyze(TypeData* self) {
  Bool _Bool118;
  CHECK(129, List_m_is_empty(self->_base._base.variables, &(_Bool118)) )
  if (!(NULL != self->base_type) && _Bool118) {
...
Returncode TypeData_write_dynamic_init(TypeData* self, TypeData* type_data) {
  ...
  while (true) {
    ...
    SyntaxTreeFunction* method = NULL;
    Int _Int119;
    CHECK(214, TypeData_find_meth(self, ((SyntaxTreeFunction*)(child->value))->name, &(method), &(_Int119)) )
    if (not_first) {
