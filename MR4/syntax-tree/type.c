...
Returncode TypeInstance_copy_new(TypeInstance* self, TypeInstance** type_instance) {
  ...
  CHECK(268, TypeData_m_new_type_instance(self->type_data, &((*type_instance))) )
  CHECK(269, string_new_copy(self->name, &((*type_instance)->name)) )
  (*type_instance)->length = self->length;
  if (NULL != self->sub_type) {
    CHECK(272, TypeInstance_copy_new(self->sub_type, &((*type_instance)->sub_type)) )
  }
  if (NULL != self->arguments) {
    CHECK(274, FunctionArguments_copy_new(self->arguments, &((*type_instance)->arguments)) )
  }
  return OK;
}
#undef MR_FUNC_NAME
...
Returncode TypeInstance_parse(TypeInstance* self, String* ends, SyntaxTreeNode* node, SyntaxTreeCode* code_node, Char* end) {
  String* name = NULL;
  String* _String118;
  CHECK(282, string_new_concat(ends, &(String){2, 1, "{"}, &(_String118)) )
  Int _Int119;
  CHECK(282, read_until(_String118, false, &(name), &((*end)), &(_Int119)) )
  CHECK(284, TypeInstance_parse_name(self, name, node, code_node, &((*end))) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
...
Returncode TypeInstance_parse_name(TypeInstance* self, String* name, SyntaxTreeNode* node, SyntaxTreeCode* code_node, Char* end) {
  CHECK(291, string_new_copy(name, &(self->name)) )
  if ((*end) == '{') {
    Bool _Bool120;
    CHECK(293, String_equal(self->name, &(String){5, 4, "Func"}, &(_Bool120)) )
    if (_Bool120) {
      CHECK(294, read_c(&((*end))) )
      if ((*end) != '(') {
        CHECK(296, SyntaxTreeNode_m_syntax_error_c(node, &(String){39, 38, "expected \"(\" inside Function type, got"}, (*end)) )
      }
      self->arguments = malloc(sizeof(FunctionArguments));
      if (self->arguments == NULL) RAISE(298)
      *self->arguments = (FunctionArguments){FunctionArguments__dtl, NULL, 0, NULL, NULL};
      ...
      CHECK(300, FunctionArguments_parse(self->arguments, &(argument_factory->_base), NULL, &((*end))) )
    }
    else {
      Bool _Bool121;
      CHECK(301, String_equal(self->name, &(String){7, 6, "String"}, &(_Bool121)) )
      if (NULL != code_node && _Bool121) {
        CHECK(302, parse_new_expression(&(String){2, 1, "}"}, code_node, &(self->length_owner), &((*end))) )
        self->length = self->length_owner;
      }
      else {
        Bool has_sub_type = true;
        Bool _Bool122;
        CHECK(307, String_equal(self->name, &(String){6, 5, "Array"}, &(_Bool122)) )
        if (NULL != code_node && _Bool122) {
          CHECK(308, parse_new_expression(&(String){3, 2, ":}"}, code_node, &(self->length_owner), &((*end))) )
          self->length = self->length_owner;
          if ((*end) != ':' && (*end) != '}') {
            CHECK(312, SyntaxTreeNode_m_syntax_error_c(node, &(String){18, 17, "expected \":\", got"}, (*end)) )
          }
          has_sub_type = (*end) == ':';
        }
        if (has_sub_type) {
          self->sub_type = malloc(sizeof(TypeInstance));
          if (self->sub_type == NULL) RAISE(315)
          *self->sub_type = (TypeInstance){NULL, NULL, NULL, NULL, NULL, NULL};
          CHECK(316, TypeInstance_parse(self->sub_type, &(String){2, 1, "}"}, node, code_node, &((*end))) )
        }
      }
    }
    if ((*end) != '}') {
      CHECK(318, SyntaxTreeNode_m_syntax_error_c(node, &(String){18, 17, "expected \"}\", got"}, (*end)) )
    }
    CHECK(319, read_c(&((*end))) )
  }
  return OK;
}
#undef MR_FUNC_NAME
...
Returncode TypeInstance_analyze(TypeInstance* self, SyntaxTreeNode* node) {
  CHECK(322, TypeInstance_analyze_part(self, node, true, true) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
...
Returncode TypeInstance_link_types(TypeInstance* self, SyntaxTreeNode* node) {
  CHECK(325, TypeInstance_analyze_part(self, node, true, false) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
...
Returncode TypeInstance_analyze_lengths(TypeInstance* self, SyntaxTreeNode* node) {
  CHECK(328, TypeInstance_analyze_part(self, node, false, true) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
...
Returncode TypeInstance_analyze_part(TypeInstance* self, SyntaxTreeNode* node, Bool check_types, Bool check_length) {
  if (check_types) {
    if (!(NULL != self->type_data)) {
      CHECK(334, SyntaxTreeNode_find_type(node, self->name, &(self->type_data)) )
    }
    if (NULL != self->arguments) {
      CHECK(336, (self->arguments)->_base._dtl[0](self->arguments) )
    }
  }
  if (check_length) {
    if (NULL != self->length) {
      CHECK(339, SyntaxTreeNode_analyze_expression(node, self->length, &(glob->type_int->_base)) )
    }
    if (NULL != self->arguments) {
      CHECK(341, (self->arguments)->_base._dtl[1](self->arguments) )
    }
  }
  if (NULL != self->sub_type) {
    CHECK(343, TypeInstance_analyze_part(self->sub_type, node, check_types, check_length) )
  }
  if (check_types) {
    if (self->type_data == &(glob->type_array->_base)) {
      if (!(NULL != self->sub_type)) {
        CHECK(347, SyntaxTreeNode_m_syntax_error_msg(node, &(String){26, 25, "missing subtype for array"}) )
      }
      if (self->sub_type->type_data == &(glob->type_array->_base)) {
        CHECK(349, SyntaxTreeNode_m_syntax_error_msg(node, &(String){44, 43, "multidimensional array not supported yet..."}) )
      }
    }
    else {
      if (self->type_data == &(glob->type_func->_base) &&  ! (NULL != self->arguments)) {
        CHECK(352, SyntaxTreeNode_m_syntax_error_msg(node, &(String){35, 34, "missing arguments in function type"}) )
      }
    }
  }
  return OK;
...
Returncode TypeInstance_check_assign_from(TypeInstance* self, SyntaxTreeNode* node, Expression** value) {
  Int bases = 0;
  CHECK(357, TypeInstance_check_assign_to((*value)->result_type, self, node, &(bases)) )
  if (bases > 0) {
    CHECK(359, UpCastExpression_init_new(NULL, bases, &((*value))) )
  }
  return OK;
}
#undef MR_FUNC_NAME
...
Returncode TypeInstance_check_assign_to(TypeInstance* self, TypeInstance* target, SyntaxTreeNode* node, Int* bases) {
  (*bases) = 0;
  if (!(NULL != self)) {
    CHECK(365, SyntaxTreeNode_m_syntax_error_msg(node, &(String){30, 29, "cannot assign void expression"}) )
  }
  if (!(NULL != target)) {
    CHECK(367, SyntaxTreeNode_m_syntax_error_msg(node, &(String){35, 34, "cannot assign into void expression"}) )
  }
  ...
  while (true) {
    Bool _Bool123;
    CHECK(373, TypeData_m_is_same(target->type_data, base_type, &(_Bool123)) )
    if (!(!_Bool123)) break;
    if (!(NULL != base_type->base_type)) {
      CHECK(375, SyntaxTreeNode_m_syntax_error2(node, &(String){14, 13, "cannot assign"}, self->type_data->name, &(String){5, 4, "into"}, target->type_data->name) )
    }
  ...
  if (NULL != self->arguments) {
    Bool _Bool124;
    CHECK(384, FunctionArguments_check_same_as(self->arguments, target->arguments, &(_Bool124)) )
    if (_Bool124) {
      CHECK(385, SyntaxTreeNode_m_syntax_error_msg(node, &(String){16, 15, "too few outputs"}) )
    }
  }
  
  CHECK(387, TypeInstance_check_sub_equal(self, target, node) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
...
Returncode TypeInstance_check_equal(TypeInstance* self, TypeInstance* other, SyntaxTreeNode* node) {
  if (self->type_data != other->type_data) {
    CHECK(391, SyntaxTreeNode_m_syntax_error2(node, &(String){19, 18, "non matching types"}, self->type_data->name, &(String){4, 3, "and"}, other->type_data->name) )
  }
  CHECK(396, TypeInstance_check_sub_equal(self, other, node) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
  ...
  if (NULL != self->sub_type) {
    if (NULL != other->sub_type) {
      if (self->sub_type->type_data != other->sub_type->type_data) {
        CHECK(402, SyntaxTreeNode_m_syntax_error2(node, &(String){22, 21, "non matching subtypes"}, self->sub_type->type_data->name, &(String){4, 3, "and"}, other->sub_type->type_data->name) )
      }
      CHECK(407, TypeInstance_check_sub_equal(self->sub_type, other->sub_type, node) )
    }
    else {
      CHECK(409, SyntaxTreeNode_m_syntax_error2(node, &(String){22, 21, "non matching subtypes"}, self->sub_type->type_data->name, &(String){25, 24, "and type with no subtype"}, other->type_data->name) )
    }
  }
  else {
    if (NULL != other->sub_type) {
      CHECK(415, SyntaxTreeNode_m_syntax_error2(node, &(String){34, 33, "non matching type with no subtype"}, self->type_data->name, &(String){12, 11, "and subtype"}, other->sub_type->type_data->name) )
    }
  }
...
Returncode TypeInstance_check_sequence(TypeInstance* self, SyntaxTreeNode* node) {
  ...
  if (self->type_data == &(glob->type_array->_base) && self->sub_type->type_data == &(glob->type_string->_base) &&  ! (NULL != self->sub_type->length)) {
    CHECK(428, SyntaxTreeNode_m_syntax_error_msg(node, &(String){28, 27, "missing length for sequence"}) )
  }
  return OK;
}
#undef MR_FUNC_NAME
...
Returncode TypeInstance_write(TypeInstance* self) {
  CHECK(431, write(self->type_data->name) )
  if (NULL != self->sub_type) {
    CHECK(433, write(&(String){2, 1, "{"}) )
    CHECK(434, TypeInstance_write(self->sub_type) )
    CHECK(435, write(&(String){2, 1, "}"}) )
  }
  return OK;
}
#undef MR_FUNC_NAME
