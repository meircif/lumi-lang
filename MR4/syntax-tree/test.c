...
Returncode SyntaxTreeMockFunction_analyze(SyntaxTreeMockFunction* self) {
  ...
  if (NULL != self->type_name) {
    ...
    Int _Int109;
    CHECK(112, TypeData_find_meth(self->_base.parent_type, self->mocked_name, &(mocked_func), &(_Int109)) )
    if (!(NULL != mocked_func)) {
    ...
    CHECK(119, TypeData_m_new_type_instance(self->_base.parent_type, &(_TypeInstance110)) )
    CHECK(119, FunctionArguments_add_self_parameter(self->_base.arguments, _TypeInstance110) )
  }
  else {
  ...
  Bool _Bool111;
  CHECK(126, FunctionArguments_check_same_as(self->_base.arguments, mocked_func->arguments, &(_Bool111)) )
