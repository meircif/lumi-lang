...
Returncode SyntaxTreeBranch_parse_children(SyntaxTreeBranch* self, TypeData* parent_type, SyntaxTreeBlock* parent_block, Char* end) {
  ...
  while (true) {
    ...
    if (keyword->length > 0 || (*end) != '\n') {
      ...
      Bool _Bool72;
      CHECK(33, SyntaxTreeBranch_parse_if_comment(self, keyword, &((*end)), &(_Bool72)) )
      if (!_Bool72) {
        ...
        Bool _Bool73;
        CHECK(47, SyntaxTreeBranch_parse_if_common(self, keyword, parent_type, parent_block, &((*end)), &(_Bool73)) )
        if (!_Bool73) {
...
Returncode SyntaxTreeBranch_parse_if_comment(SyntaxTreeBranch* self, String* keyword, Char* end, Bool* is_parsed) {
  ...
  Bool _Bool74;
  CHECK(63, String_equal(keyword, &(String){2, 1, "#"}, &(_Bool74)) )
  Bool _Bool75;
  CHECK(63, String_equal(keyword, &(String){3, 2, "##"}, &(_Bool75)) )
  if (_Bool74 || _Bool75) {
    String* text = NULL;
    Int _Int76;
    CHECK(65, read_until(&(String){1, 0, ""}, false, &(text), &((*end)), &(_Int76)) )
  }
  else {
    Bool _Bool77;
    CHECK(66, String_equal(keyword, &(String){3, 2, "~#"}, &(_Bool77)) )
    Bool _Bool78;
    CHECK(66, String_equal(keyword, &(String){4, 3, "~##"}, &(_Bool78)) )
    if (_Bool77 || _Bool78) {
      Char prev = '\0';
      while (true) {
        Char curr = '\0';
...
Returncode SyntaxTreeBranch_parse_if_common(SyntaxTreeBranch* self, String* keyword, TypeData* parent_type, SyntaxTreeBlock* parent_block, Char* end, Bool* is_parsed) {
  ...
  if (NULL != parent_block) {
    ...
    CHECK(86, (parent_block)->_base._base._dtl[6](parent_block, &(_SyntaxTreeFunction79)) )
  ...
  if (!(*is_parsed)) {
    ...
    Bool _Bool80;
    CHECK(91, String_equal(keyword, &(String){4, 3, "var"}, &(_Bool80)) )
    if (_Bool80) {
      SyntaxTreeVariable* _SyntaxTreeVariable81;
      CHECK(92, SyntaxTreeVariable_parse_new(NULL, ACCESS_VAR, true, parent_type, parent_block, &((*end)), &(_SyntaxTreeVariable81)) )
      CHECK(92, List_add(variables, _SyntaxTreeVariable81) )
    }
    else {
      Bool _Bool82;
      CHECK(99, String_equal(keyword, &(String){5, 4, "user"}, &(_Bool82)) )
      if (_Bool82) {
        SyntaxTreeVariable* _SyntaxTreeVariable83;
        CHECK(100, SyntaxTreeVariable_parse_new(NULL, ACCESS_USER, false, parent_type, parent_block, &((*end)), &(_SyntaxTreeVariable83)) )
        CHECK(100, List_add(variables, _SyntaxTreeVariable83) )
      }
      else {
        Bool _Bool84;
        CHECK(107, String_equal(keyword, &(String){6, 5, "owner"}, &(_Bool84)) )
        if (_Bool84) {
          SyntaxTreeVariable* _SyntaxTreeVariable85;
          CHECK(108, SyntaxTreeVariable_parse_new(NULL, ACCESS_OWNER, false, parent_type, parent_block, &((*end)), &(_SyntaxTreeVariable85)) )
          CHECK(108, List_add(variables, _SyntaxTreeVariable85) )
...
Returncode SyntaxTreeBranch_find_variable(SyntaxTreeBranch* self, String* name, SyntaxTreeVariable** variable) {
  ...
  while (true) {
    if (!(NULL != child)) break;
    Bool _Bool86;
    CHECK(128, SyntaxTreeVariable_find_variable(((SyntaxTreeVariable*)(child->item)), name, &((*variable)), &(_Bool86)) )
    if (!(!_Bool86)) break;
    child = child->next;
  }
  return OK;
...
Returncode SyntaxTreeNamespace_parse_if_function(SyntaxTreeNamespace* self, String* keyword, TypeData* parent_type, Char* end, Bool* is_func) {
  ...
  if ((*is_func)) {
    ...
    CHECK(160, SyntaxTreeFunction_parse_new(NULL, parent_type, &((*end)), &(_SyntaxTreeFunction87)) )
    CHECK(160, List_add(self->functions, _SyntaxTreeFunction87) )
  }
  return OK;
}
#undef MR_FUNC_NAME
...
Returncode SyntaxTreeNamespace_find_function(SyntaxTreeNamespace* self, String* name, SyntaxTreeFunction** function) {
  ...
  while (true) {
    if (!(NULL != child)) break;
    Bool _Bool88;
    CHECK(167, String_equal(((SyntaxTreeFunction*)(child->item))->name, name, &(_Bool88)) )
    if (_Bool88) {
