...
Returncode SyntaxTreeBranch_parse_children(SyntaxTreeBranch* self, TypeData* parent_type, SyntaxTreeBlock* parent_block, Char* end) {
  ...
  while (true) {
    ...
    if (keyword->length > 0 || (*end) != '\n') {
      ...
      Bool _Bool57;
      CHECK(33, SyntaxTreeBranch_parse_if_comment(self, keyword, &((*end)), &(_Bool57)) )
      if (!_Bool57) {
        ...
        Bool _Bool58;
        CHECK(47, SyntaxTreeBranch_parse_if_common(self, keyword, parent_type, parent_block, &((*end)), &(_Bool58)) )
        if (!_Bool58) {
          CHECK(49, (self)->_base._dtl[2](self, keyword, &((*end))) )
        }
      }
...
Returncode SyntaxTreeBranch_parse_if_comment(SyntaxTreeBranch* self, String* keyword, Char* end, Bool* is_parsed) {
  ...
  Bool _Bool59;
  CHECK(63, String_equal(keyword, &(String){2, 1, "#"}, &(_Bool59)) )
  Bool _Bool60;
  CHECK(63, String_equal(keyword, &(String){3, 2, "##"}, &(_Bool60)) )
  if (_Bool59 || _Bool60) {
    String* text = NULL;
    Int _Int61;
    CHECK(65, read_until(&(String){1, 0, ""}, false, &(text), &((*end)), &(_Int61)) )
  }
  else {
    Bool _Bool62;
    CHECK(66, String_equal(keyword, &(String){3, 2, "{#"}, &(_Bool62)) )
    Bool _Bool63;
    CHECK(66, String_equal(keyword, &(String){4, 3, "{##"}, &(_Bool63)) )
    if (_Bool62 || _Bool63) {
      Char prev = '\0';
      while (true) {
        Char curr = '\0';
...
Returncode SyntaxTreeBranch_parse_if_common(SyntaxTreeBranch* self, String* keyword, TypeData* parent_type, SyntaxTreeBlock* parent_block, Char* end, Bool* is_parsed) {
  ...
  if (!(*is_parsed)) {
    ...
    Bool _Bool64;
    CHECK(87, String_equal(keyword, &(String){4, 3, "var"}, &(_Bool64)) )
    if (_Bool64) {
      SyntaxTreeVariable* _SyntaxTreeVariable65;
      CHECK(88, SyntaxTreeVariable_parse_new(NULL, ACCESS_VAR, parent_type, parent_block, &((*end)), &(_SyntaxTreeVariable65)) )
      CHECK(88, List_add(self->variables, _SyntaxTreeVariable65) )
    }
    else {
      Bool _Bool66;
      CHECK(91, String_equal(keyword, &(String){5, 4, "user"}, &(_Bool66)) )
      if (_Bool66) {
        SyntaxTreeVariable* _SyntaxTreeVariable67;
        CHECK(92, SyntaxTreeVariable_parse_new(NULL, ACCESS_USER, parent_type, parent_block, &((*end)), &(_SyntaxTreeVariable67)) )
        CHECK(92, List_add(self->variables, _SyntaxTreeVariable67) )
      }
      else {
        Bool _Bool68;
        CHECK(95, String_equal(keyword, &(String){6, 5, "owner"}, &(_Bool68)) )
        if (_Bool68) {
          SyntaxTreeVariable* _SyntaxTreeVariable69;
          CHECK(96, SyntaxTreeVariable_parse_new(NULL, ACCESS_OWNER, parent_type, parent_block, &((*end)), &(_SyntaxTreeVariable69)) )
          CHECK(96, List_add(self->variables, _SyntaxTreeVariable69) )
...
Returncode SyntaxTreeBranch_m_find_variable(SyntaxTreeBranch* self, String* name, SyntaxTreeVariable** variable) {
  ...
  while (true) {
    if (!(NULL != child)) break;
    Bool _Bool70;
    CHECK(111, SyntaxTreeVariable_m_find_variable(((SyntaxTreeVariable*)(child->item)), name, &((*variable)), &(_Bool70)) )
    if (!(!_Bool70)) break;
    child = child->next;
  }
  (*variable) = NULL;
...
Returncode SyntaxTreeBranch_analyze(SyntaxTreeBranch* self) {
  CHECK(116, SyntaxTreeBranch_analyze_children(self, self->variables) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
...
Returncode SyntaxTreeBranch_analyze_children(SyntaxTreeBranch* self, List* child_list) {
  ...
  while (true) {
    if (!(NULL != child)) break;
    CHECK(122, (((SyntaxTreeNode*)(child->item)))->_dtl[0](((SyntaxTreeNode*)(child->item))) )
    child = child->next;
  }
  return OK;
}
...
Returncode SyntaxTreeBranch_write(SyntaxTreeBranch* self) {
  CHECK(126, SyntaxTreeBranch_write_children(self, self->variables) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
...
Returncode SyntaxTreeBranch_write_children(SyntaxTreeBranch* self, List* child_list) {
  ...
  while (true) {
    ...
    if (self->indentation_spaces > 0) {
      CHECK(133, SyntaxTreeBranch_write_spaces(self) )
    }
    else {
      CHECK(135, write(&(String){2, 1, "\n"}) )
    }
    CHECK(136, (((SyntaxTreeNode*)(child->item)))->_dtl[1](((SyntaxTreeNode*)(child->item))) )
...
Returncode SyntaxTreeNamespace_parse_if_function(SyntaxTreeNamespace* self, String* keyword, TypeData* parent_type, Char* end, Bool* is_func) {
  ...
  if ((*is_func)) {
    ...
    CHECK(160, SyntaxTreeFunction_parse_new(NULL, parent_type, &((*end)), &(_SyntaxTreeFunction71)) )
    CHECK(160, List_add(self->functions, _SyntaxTreeFunction71) )
  }
  return OK;
}
#undef MR_FUNC_NAME
...
Returncode SyntaxTreeNamespace_m_find_function(SyntaxTreeNamespace* self, String* name, SyntaxTreeFunction** function) {
  ...
  while (true) {
    if (!(NULL != child)) break;
    Bool _Bool72;
    CHECK(167, String_equal(((SyntaxTreeFunction*)(child->item))->name, name, &(_Bool72)) )
    if (_Bool72) {
...
Returncode SyntaxTreeNamespace_analyze(SyntaxTreeNamespace* self) {
  ...
  CHECK(175, SyntaxTreeBranch_analyze_children(&(self->_base), self->functions) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
