...
Returncode SyntaxTreeNamespace_parse_if_function(SyntaxTreeNamespace* self, String* keyword, TypeData* parent_type, Char* end, Bool* is_func) {
  ...
  if ((*is_func)) {
    ...
    CHECK(160, SyntaxTreeFunction_parse_new(NULL, parent_type, &((*end)), &(_SyntaxTreeFunction88)) )
    CHECK(160, List_add(self->functions, _SyntaxTreeFunction88) )
  }
  return OK;
}
#undef MR_FUNC_NAME
...
Returncode SyntaxTreeNamespace_find_function(SyntaxTreeNamespace* self, String* name, SyntaxTreeFunction** function) {
  ...
  while (true) {
    if (!(NULL != child)) break;
    Bool _Bool89;
    CHECK(167, String_equal(((SyntaxTreeFunction*)(child->item))->name, name, &(_Bool89)) )
    if (_Bool89) {
