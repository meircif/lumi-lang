  ...
  while (true) {
    ...
    if (keyword->length > 0 || (*end) != '\n') {
      ...
      if (!_Bool73) {
        ...
        Bool _Bool74;
        CHECK(47, SyntaxTreeBranch_parse_if_common(self, keyword, parent_type, parent_block, &((*end)), &(_Bool74)) )
        if (!_Bool74) {
...
Returncode SyntaxTreeBranch_parse_if_comment(SyntaxTreeBranch* self, String* keyword, Char* end, Bool* is_parsed) {
  ...
  Bool _Bool75;
  CHECK(63, String_equal(keyword, &(String){2, 1, "#"}, &(_Bool75)) )
  Bool _Bool76;
  CHECK(63, String_equal(keyword, &(String){3, 2, "##"}, &(_Bool76)) )
  if (_Bool75 || _Bool76) {
    String* text = NULL;
    Int _Int77;
    CHECK(65, read_until(&(String){1, 0, ""}, false, &(text), &((*end)), &(_Int77)) )
  }
  else {
    Bool _Bool78;
    CHECK(66, String_equal(keyword, &(String){3, 2, "~#"}, &(_Bool78)) )
    Bool _Bool79;
    CHECK(66, String_equal(keyword, &(String){4, 3, "~##"}, &(_Bool79)) )
    if (_Bool78 || _Bool79) {
      Char prev = '\0';
      while (true) {
        Char curr = '\0';
...
Returncode SyntaxTreeBranch_parse_if_common(SyntaxTreeBranch* self, String* keyword, TypeData* parent_type, SyntaxTreeBlock* parent_block, Char* end, Bool* is_parsed) {
  ...
  if (NULL != parent_block) {
    ...
    CHECK(86, (parent_block)->_base._base._dtl[6](parent_block, &(_SyntaxTreeFunction80)) )
    variables = _SyntaxTreeFunction80->_base._base.variables;
  }
  ...
  if (!(*is_parsed)) {
    ...
    Bool _Bool81;
    CHECK(91, String_equal(keyword, &(String){4, 3, "var"}, &(_Bool81)) )
    if (_Bool81) {
      SyntaxTreeVariable* _SyntaxTreeVariable82;
      CHECK(92, SyntaxTreeVariable_parse_new(NULL, ACCESS_VAR, true, parent_type, parent_block, &((*end)), &(_SyntaxTreeVariable82)) )
      CHECK(92, List_add(variables, _SyntaxTreeVariable82) )
    }
    else {
      Bool _Bool83;
      CHECK(99, String_equal(keyword, &(String){5, 4, "user"}, &(_Bool83)) )
      if (_Bool83) {
        SyntaxTreeVariable* _SyntaxTreeVariable84;
        CHECK(100, SyntaxTreeVariable_parse_new(NULL, ACCESS_USER, false, parent_type, parent_block, &((*end)), &(_SyntaxTreeVariable84)) )
        CHECK(100, List_add(variables, _SyntaxTreeVariable84) )
      }
      else {
        Bool _Bool85;
        CHECK(107, String_equal(keyword, &(String){6, 5, "owner"}, &(_Bool85)) )
        if (_Bool85) {
          SyntaxTreeVariable* _SyntaxTreeVariable86;
          CHECK(108, SyntaxTreeVariable_parse_new(NULL, ACCESS_OWNER, false, parent_type, parent_block, &((*end)), &(_SyntaxTreeVariable86)) )
          CHECK(108, List_add(variables, _SyntaxTreeVariable86) )
...
Returncode SyntaxTreeBranch_find_variable(SyntaxTreeBranch* self, String* name, SyntaxTreeVariable** variable) {
  ...
  while (true) {
    if (!(NULL != child)) break;
    Bool _Bool87;
    CHECK(128, SyntaxTreeVariable_find_variable(((SyntaxTreeVariable*)(child->item)), name, &((*variable)), &(_Bool87)) )
    if (!(!_Bool87)) break;
    child = child->next;
  }
  return OK;
...
#endif
#if MR_STAGE == MR_DECLARATIONS
Returncode SyntaxTreeBranch_write_cleanup(SyntaxTreeBranch* self, Bool is_type);
#elif MR_STAGE == MR_FUNCTIONS
...
Returncode SyntaxTreeBranch_write_cleanup(SyntaxTreeBranch* self, Bool is_type) {
  ...
  while (true) {
    ...
    if (!((SyntaxTreeVariable*)(var_node->item))->type_instance->type_data->is_primitive) {
      CHECK(148, SyntaxTreeBranch_write_variable_cleanup(self, var_node->item, is_type) )
    }
...
#endif
#if MR_STAGE == MR_DECLARATIONS
Returncode SyntaxTreeBranch_write_variable_cleanup(SyntaxTreeBranch* self, SyntaxTreeVariable* variable, Bool is_type);
#elif MR_STAGE == MR_FUNCTIONS
...
Returncode SyntaxTreeBranch_write_variable_cleanup(SyntaxTreeBranch* self, SyntaxTreeVariable* variable, Bool is_type) {
  ...
  if (variable->access == ACCESS_OWNER &&  ! item_type_data->is_primitive && item_type_data != &(glob->type_string->_base) && item_type_data != &(glob->type_file->_base) && item_type_data != &(glob->type_generic->_base)) {
    CHECK(160, write(&(String){3, 2, "  "}) )
    if (variable->type_instance->type_data->is_dynamic) {
      if (is_type) {
        CHECK(163, write(&(String){7, 6, "self->"}) )
      }
      CHECK(164, write_cname(variable->name) )
      CHECK(165, write(&(String){11, 10, "_Dynamic->"}) )
      TypeData* type_data = item_type_data;
      while (true) {
        if (!(NULL != type_data->base_type)) break;
        type_data = type_data->base_type->type_data;
        if (!(type_data->is_dynamic)) break;
        CHECK(171, write(&(String){7, 6, "_base."}) )
      }
      CHECK(172, write(&(String){6, 5, "_del("}) )
    }
    else {
      if (variable->type_instance->type_data == &(glob->type_array->_base)) {
        CHECK(174, write(&(String){11, 10, "ARRAY_DEL("}) )
        CHECK(175, write_cname(item_type_data->name) )
        CHECK(176, write(&(String){3, 2, ", "}) )
      }
      else {
        CHECK(178, write_cname(item_type_data->name) )
        CHECK(179, write(&(String){6, 5, "_Del("}) )
      }
    }
    if (is_type) {
      CHECK(181, write(&(String){7, 6, "self->"}) )
    }
    CHECK(182, write_cname(variable->name) )
    CHECK(183, write(&(String){4, 3, ");\n"}) )
  }
  CHECK(184, write(&(String){6, 5, "  MR_"}) )
  if (variable->access == ACCESS_OWNER) {
    CHECK(186, write(&(String){7, 6, "owner_"}) )
  }
  CHECK(187, write(&(String){9, 8, "dec_ref("}) )
  if (is_type) {
    CHECK(189, write(&(String){7, 6, "self->"}) )
  }
  CHECK(190, write_cname(variable->name) )
  CHECK(191, write(&(String){11, 10, "_Refman);\n"}) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
...
Returncode SyntaxTreeNamespace_init(SyntaxTreeNamespace* self) {
  CHECK(199, SyntaxTreeBranch_init(&(self->_base)) )
  self->functions = malloc(sizeof(List));
  if (self->functions == NULL) RAISE(200)
...
Returncode SyntaxTreeNamespace_parse_if_function(SyntaxTreeNamespace* self, String* keyword, TypeData* parent_type, Char* end, Bool* is_func) {
  CHECK(205, String_equal(keyword, &(String){5, 4, "func"}, &((*is_func))) )
  if ((*is_func)) {
    if ((*end) != ' ') {
      CHECK(208, SyntaxTreeNode_m_syntax_error_c(&(self->_base._base), &(String){33, 32, "expected space after \"func\", got"}, (*end)) )
    }
    SyntaxTreeFunction* _SyntaxTreeFunction88;
    CHECK(210, SyntaxTreeFunction_parse_new(NULL, parent_type, &((*end)), &(_SyntaxTreeFunction88)) )
    CHECK(210, List_add(self->functions, _SyntaxTreeFunction88) )
  }
  return OK;
}
#undef MR_FUNC_NAME
...
Returncode SyntaxTreeNamespace_find_function(SyntaxTreeNamespace* self, String* name, SyntaxTreeFunction** function) {
  ...
  while (true) {
    if (!(NULL != child)) break;
    Bool _Bool89;
    CHECK(217, String_equal(((SyntaxTreeFunction*)(child->item))->name, name, &(_Bool89)) )
    if (_Bool89) {
...
Returncode SyntaxTreeNamespace_link_types(SyntaxTreeNamespace* self) {
  CHECK(224, SyntaxTreeBranch_link_types(&(self->_base)) )
  CHECK(225, SyntaxTreeNode_link_children_types(&(self->_base._base), self->functions) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
...
Returncode SyntaxTreeNamespace_analyze(SyntaxTreeNamespace* self) {
  CHECK(228, SyntaxTreeBranch_analyze(&(self->_base)) )
  CHECK(229, SyntaxTreeNode_analyze_children(&(self->_base._base), self->functions) )
  return OK;
}
#undef MR_FUNC_NAME
#endif
...
Returncode SyntaxTreeNamespace_write_functions_declaration(SyntaxTreeNamespace* self) {
  ...
  while (true) {
    if (!(NULL != child)) break;
    CHECK(235, (((SyntaxTreeFunction*)(child->item)))->_base._base._base._dtl[8](((SyntaxTreeFunction*)(child->item))) )
    child = child->next;
  }
  return OK;
}
