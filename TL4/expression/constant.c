...
Returncode IntExpression_parse(IntExpression* self, String* text) {
  ...
  if (((number)->values[0]) == '0' && number->length > 1) {
    ...
    if (second >= '0' && second <= '7') {
      Bool _Bool34;
      CHECK(59, f_is_octal(number, &(_Bool34)) )
      if (!_Bool34) {
      ...
      if (second == 'b' || second == 'B') {
        Bool _Bool35;
        CHECK(64, f_is_binary(number, &(_Bool35)) )
        if (!_Bool35) {
        ...
        if (second == 'x' || second == 'X') {
          Bool _Bool36;
          CHECK(69, f_is_hex(number, &(_Bool36)) )
          if (!_Bool36) {
    ...
    Bool _Bool37;
    CHECK(73, f_is_decimal(number, &(_Bool37)) )
    if (!_Bool37) {
...
Returncode CharExpression_parse(CharExpression* self, String* text) {
  ...
  if (text->length == 3) {
    ...
    if (text->length == 4) {
      ...
      Bool _Bool38;
      CHECK(99, String_has(&(String){13, 12, "'\"?\\abfnrtv0"}, ((text)->values[2]), &(_Bool38)) )
      if (!_Bool38) {
      ...
      if (text->length == 6) {
        ...
        if (ch == 'x') {
          ...
          Bool _Bool39;
          CHECK(106, f_is_hex((&(String){2, 2, (text)->values + (3)}), &(_Bool39)) )
          if (!_Bool39) {
          ...
          Bool _Bool40;
          CHECK(109, f_is_octal((&(String){3, 3, (text)->values + (2)}), &(_Bool40)) )
          if (!_Bool40) {
