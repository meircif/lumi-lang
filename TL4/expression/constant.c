...
Returncode IntExpression_parse(IntExpression* self, String* text) {
  ...
  if (((number)->values[0]) == '0' && number->length > 1) {
    ...
    if (second >= '0' && second <= '7') {
      Bool _Bool38;
      CHECK(59, f_is_octal(number, &(_Bool38)) )
      if (!_Bool38) {
      ...
      if (second == 'b' || second == 'B') {
        Bool _Bool39;
        CHECK(64, f_is_binary(number, &(_Bool39)) )
        if (!_Bool39) {
        ...
        if (second == 'x' || second == 'X') {
          Bool _Bool40;
          CHECK(69, f_is_hex(number, &(_Bool40)) )
          if (!_Bool40) {
    ...
    Bool _Bool41;
    CHECK(73, f_is_decimal(number, &(_Bool41)) )
    if (!_Bool41) {
...
Returncode CharExpression_parse(CharExpression* self, String* text) {
  ...
  if (text->length == 3) {
    ...
    if (text->length == 4) {
      ...
      Bool _Bool42;
      CHECK(99, String_has(&(String){13, 12, "'\"?\\abfnrtv0"}, ((text)->values[2]), &(_Bool42)) )
      if (!_Bool42) {
      ...
      if (text->length == 6) {
        ...
        if (ch == 'x') {
          ...
          Bool _Bool43;
          CHECK(106, f_is_hex((&(String){2, 2, (text)->values + (3)}), &(_Bool43)) )
          if (!_Bool43) {
          ...
          Bool _Bool44;
          CHECK(109, f_is_octal((&(String){3, 3, (text)->values + (2)}), &(_Bool44)) )
          if (!_Bool44) {
