...
static char* _lumi_file13_name = "expression/expression.3.lm";
#endif
...
Returncode parse_new_expression(String* ends, SyntaxTreeCode* code_node, Expression** expression, Char* end) {
  Operator* _Operator49;
  CHECK(23, Expression_parse_new(NULL, ends, code_node, NULL, &((*expression)), &((*end)), &(_Operator49)) )
  return OK;
}
...
Returncode Expression_parse_new_operand(Expression* self, String* text, String* ends, SyntaxTreeCode* code_node, Expression** expression, Char* end) {
  ...
  while (true) {
    ...
    Bool _Bool50;
    CHECK(110, Expression_parse_new_follow_operand(self, ends, code_node, &((*expression)), &((*end)), &(_Bool50)) )
    if (!(_Bool50)) break;
  }
  return OK;
}
      ...
      if ((first >= '0' && first <= '9') || (first == '-' && second >= '0' && second <= '9')) {
        ...
        if (first >= 'A' && first <= 'Z' && second >= 'a' && second <= 'z') {
          ...
          if (text->length == 1 && first == '_') {
            ...
            Bool _Bool51;
            CHECK(134, String_equal(text, &(String){5, 4, "base"}, &(_Bool51)) )
            if (_Bool51) {
...
Returncode Expression_add_aux_variable(Expression* self, Int access, Bool is_create, TypeInstance* type_instance, SymbolExpression** symbol) {
  ...
  *(*symbol) = (SymbolExpression){SymbolExpression__dtl, NULL, 0, NULL, NULL, 0, false, false, false, false, false, false, NULL, NULL, NULL, NULL, false};
  ...
  CHECK(180, SyntaxTreeCode_get_function(self->code_node, &(_SyntaxTreeFunction52)) )
  CHECK(180, SyntaxTreeFunction_add_aux_variable(_SyntaxTreeFunction52, access, is_create, type_instance, &((*symbol)->variable)) )
...
Returncode Expression_write_assign_null(Expression* self) {
  CHECK(211, (self)->_base._dtl[4](self) )
  CHECK(212, write(&(String){10, 9, " = NULL;\n"}) )
  CHECK(213, SyntaxTreeCode_write_spaces(self->code_node) )
  CHECK(214, (self)->_base._dtl[6](self) )
  CHECK(215, write(&(String){10, 9, " = NULL;\n"}) )
  if (self->result_type->type_data->is_dynamic) {
    CHECK(217, SyntaxTreeCode_write_spaces(self->code_node) )
    CHECK(218, (self)->_base._dtl[5](self) )
    CHECK(219, write(&(String){10, 9, " = NULL;\n"}) )
  }
...
Returncode Expression_write_check_ref(Expression* self) {
  ...
  CHECK(225, write(&(String){5, 4, "if ("}) )
  CHECK(226, Expression_write_as_top(self) )
  CHECK(227, write(&(String){11, 10, " == NULL) "}) )
  CHECK(228, SyntaxTreeNode_write_raise(&(self->_base), &(String){18, 17, "empty object used"}) )
  CHECK(229, SyntaxTreeCode_write_spaces(self->code_node) )
  CHECK(230, write(&(String){5, 4, "if ("}) )
  CHECK(231, (self)->_base._dtl[6](self) )
  CHECK(232, write(&(String){18, 17, "->value == NULL) "}) )
  CHECK(233, SyntaxTreeNode_write_raise(&(self->_base), &(String){29, 28, "outdated weak reference used"}) )
  return OK;
}
#undef LUMI_FUNC_NAME
#endif
...
Returncode Expression_write_dynamic(Expression* self) {
  CHECK(236, (self)->_base._dtl[4](self) )
  CHECK(237, write(&(String){9, 8, "_Dynamic"}) )
  return OK;
}
#undef LUMI_FUNC_NAME
#endif
...
Returncode Expression_write_refman(Expression* self) {
  CHECK(240, (self)->_base._dtl[4](self) )
  CHECK(241, write(&(String){8, 7, "_Refman"}) )
  return OK;
}
#undef LUMI_FUNC_NAME
#endif
...
Returncode Expression_write_as_top(Expression* self) {
  Bool top = self->top;
  self->top = true;
  CHECK(246, (self)->_base._dtl[4](self) )
  self->top = top;
  return OK;
}
#undef LUMI_FUNC_NAME
