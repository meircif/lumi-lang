...
Returncode SyntaxTreeBranch_parse_children(SyntaxTreeBranch* self, TypeData* parent_type, SyntaxTreeBlock* parent_block, Char* end) {
  ...
  while (true) {
    ...
    if (keyword->length > 0 || (*end) != '\n') {
      ...
      Bool _Bool83;
      CHECK(33, SyntaxTreeBranch_parse_if_comment(self, keyword, &((*end)), &(_Bool83)) )
      if (!_Bool83) {
        ...
        Bool _Bool84;
        CHECK(40, (self)->_base._dtl[5](self, keyword, parent_type, parent_block, &((*end)), &(_Bool84)) )
        if (!_Bool84) {
...
Returncode SyntaxTreeBranch_parse_if_comment(SyntaxTreeBranch* self, String* keyword, Char* end, Bool* is_parsed) {
  (*is_parsed) = true;
  Bool _Bool85;
  CHECK(54, String_equal(keyword, &(String){2, 1, ";"}, &(_Bool85)) )
  if (_Bool85 && (*end) == ' ') {
    String* text = NULL;
    Int _Int86;
    CHECK(56, read_until(&(String){1, 0, ""}, false, &(text), &((*end)), &(_Int86)) )
  }
  else {
    Bool _Bool87;
    CHECK(57, String_equal(keyword, &(String){4, 3, "~~~"}, &(_Bool87)) )
    if (_Bool87 && (*end) == ' ') {
      Int count = 0;
      while (true) {
        Char ch = '\0';
...
Returncode SyntaxTreeBranch_parse_if_common(SyntaxTreeBranch* self, String* keyword, TypeData* parent_type, SyntaxTreeBlock* parent_block, Char* end, Bool* is_parsed) {
  ...
  if (NULL != parent_block) {
    ...
    CHECK(91, (parent_block)->_base._base._dtl[8](parent_block, &(_SyntaxTreeFunction89)) )
    variables = _SyntaxTreeFunction89->_base._base.variables;
    if (NULL != glob->tested_module) {
      Bool _Bool90;
      CHECK(93, String_equal(glob->current_module->name, glob->tested_module, &(_Bool90)) )
      Bool _Bool91;
      CHECK(93, String_equal(keyword, &(String){5, 4, "else"}, &(_Bool91)) )
      Bool _Bool92;
      CHECK(93, String_equal(keyword, &(String){8, 7, "else-if"}, &(_Bool92)) )
      if (_Bool90 &&  ! _Bool91 &&  ! _Bool92) {
        SyntaxTreeCoverage* _SyntaxTreeCoverage93;
        CHECK(95, SyntaxTreeCoverage_init_new(NULL, parent_block, &(_SyntaxTreeCoverage93)) )
        CHECK(95, List_add(parent_block->code_nodes, &(_SyntaxTreeCoverage93->_base)) )
      }
    }
  }
  ...
  if (!(*is_parsed)) {
    ...
    Bool _Bool94;
    CHECK(101, String_equal(keyword, &(String){4, 3, "var"}, &(_Bool94)) )
    if (_Bool94) {
      SyntaxTreeVariable* _SyntaxTreeVariable95;
      CHECK(102, SyntaxTreeVariable_parse_new(NULL, ACCESS_VAR, true, parent_type, parent_block, &((*end)), &(_SyntaxTreeVariable95)) )
      CHECK(102, List_add(variables, _SyntaxTreeVariable95) )
    }
    else {
      Bool _Bool96;
      CHECK(109, String_equal(keyword, &(String){5, 4, "user"}, &(_Bool96)) )
      if (_Bool96) {
        SyntaxTreeVariable* _SyntaxTreeVariable97;
        CHECK(110, SyntaxTreeVariable_parse_new(NULL, ACCESS_USER, false, parent_type, parent_block, &((*end)), &(_SyntaxTreeVariable97)) )
        CHECK(110, List_add(variables, _SyntaxTreeVariable97) )
      }
      else {
        Bool _Bool98;
        CHECK(117, String_equal(keyword, &(String){6, 5, "owner"}, &(_Bool98)) )
        if (_Bool98) {
          SyntaxTreeVariable* _SyntaxTreeVariable99;
          CHECK(118, SyntaxTreeVariable_parse_new(NULL, ACCESS_OWNER, false, parent_type, parent_block, &((*end)), &(_SyntaxTreeVariable99)) )
          CHECK(118, List_add(variables, _SyntaxTreeVariable99) )
...
Returncode SyntaxTreeNamespace_parse_if_function(SyntaxTreeNamespace* self, String* keyword, TypeData* parent_type, Char* end, Bool* is_func) {
  ...
  if ((*is_func)) {
    ...
    SyntaxTreeFunction* _SyntaxTreeFunction100;
    CHECK(255, SyntaxTreeFunction_parse_new(NULL, parent_type, &((*end)), &(_SyntaxTreeFunction100)) )
    CHECK(255, List_add(self->functions, _SyntaxTreeFunction100) )
  }
  return OK;
}
#undef LUMI_FUNC_NAME
