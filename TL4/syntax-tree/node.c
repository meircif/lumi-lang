...
Returncode SyntaxTreeNode_get_access(SyntaxTreeNode* self, String* access_str, Int* access) {
  {int n; for (n = (1); n < (5); ++n) {
    ...
    Bool _Bool110;
    CHECK(23, String_equal((&(((String*)((glob->access_names)->values))[n])), access_str, &(_Bool110)) )
    if (_Bool110) {
      (*access) = n;
      return OK;
    }
...
Returncode SyntaxTreeNode_find_type(SyntaxTreeNode* self, String* name, String* module_name, TypeData** type_data) {
  if (NULL != module_name) {
    ...
    CHECK(39, SyntaxTreeNode_find_module(self, module_name, &(_ModuleMembers111)) )
    CHECK(39, NameMap_find(_ModuleMembers111->type_map, name, (void**)&((*type_data))) )
    if (NULL != (*type_data)) {
      return OK;
    }
  ...
  if (NULL != parent_type &&  NULL !=  parent_type->parameters) {
    ...
    while (true) {
      if (!(NULL != node)) break;
      Bool _Bool112;
      CHECK(51, String_equal(((String*)(node->item)), name, &(_Bool112)) )
      if (_Bool112) {
...
Returncode SyntaxTreeNode_find_variable(SyntaxTreeNode* self, String* name, String* module_name, SyntaxTreeVariable** variable) {
  if (NULL != module_name) {
    ...
    CHECK(61, SyntaxTreeNode_find_module(self, module_name, &(_ModuleMembers113)) )
    CHECK(61, NameMap_find(_ModuleMembers113->variable_map, name, (void**)&((*variable))) )
  }
  else {
...
Returncode SyntaxTreeNode_find_function(SyntaxTreeNode* self, String* name, String* module_name, SyntaxTreeFunction** function) {
  if (NULL != module_name) {
    ...
    CHECK(69, SyntaxTreeNode_find_module(self, module_name, &(_ModuleMembers114)) )
    CHECK(69, NameMap_find(_ModuleMembers114->function_map, name, (void**)&((*function))) )
  }
  else {
...
Returncode SyntaxTreeNode_read_expect(SyntaxTreeNode* self, String* expected_text) {
  ...
  {int n; for (n = (0); n < (expected_text->length); ++n) {
    Char _Char115;
    CHECK(78, read_c(&(_Char115)) )
    CHECK(78, String_append(actual_text, _Char115) )
  }}
  Bool _Bool116;
  CHECK(79, String_equal(actual_text, expected_text, &(_Bool116)) )
  if (!_Bool116) {
