...
Returncode f_assert_string_slice(String* expected, String* actual, Int start, Int slice_length, Bool is_main) {
  ...
  Bool _Bool198;
  CHECK(111, String_equal(actual_slice, expected, &(_Bool198)) )
  if (_Bool198) {
    return OK;
  }
...
Returncode set_test_file_name(String* filename, String* base_name, String* suffix) {
  ...
  Bool _Bool199;
  CHECK(137, Sys_getenv(sys, &(String){9, 8, "TEST_DIR"}, base_dir, &(_Bool199)) )
  if (_Bool199) {
...
Returncode test_new_file(String* base_name, String* name) {
  ...
  Bool _Bool200;
  CHECK(154, read_line(input_file, line, &(String){7, 6, "## @@ "}, &(_Bool200)) )
  if (!_Bool200) {
    CHECK(155, file_error(name, line) )
  }
  Bool _Bool201;
  CHECK(156, read_line(expected_output_file, line, &(String){8, 7, "/// @@ "}, &(_Bool201)) )
  if (!_Bool201) {
...
Returncode test_from_file(String* name) {
  ...
  if (had_test_error) {
    while (true) {
      Bool _Bool202;
      CHECK(166, read_line(input_file, line, &(String){7, 6, "## @@ "}, &(_Bool202)) )
      if (!(!_Bool202)) break;
    }
    while (true) {
      Bool _Bool203;
      CHECK(168, read_line(expected_output_file, line, &(String){8, 7, "/// @@ "}, &(_Bool203)) )
      if (!(!_Bool203)) break;
    }
  }
  had_test_error = true;
  Bool _Bool204;
  CHECK(170, read_line(input_file, line, NULL, &(_Bool204)) )
  Bool _Bool205;
  CHECK(171, String_equal(line, name, &(_Bool205)) )
  if (!_Bool205) {
    CHECK(172, file_error(name, line) )
  }
  Bool _Bool206;
  CHECK(173, read_line(expected_output_file, line, NULL, &(_Bool206)) )
  Bool _Bool207;
  CHECK(174, String_equal(line, name, &(_Bool207)) )
  if (!_Bool207) {
  ...
  Bool _Bool208;
  CHECK(183, read_line(input_file, line, &(String){6, 5, "## @ "}, &(_Bool208)) )
  if (!_Bool208) {
    CHECK(184, file_error(name, line) )
  }
  Bool _Bool209;
  CHECK(185, read_line(expected_output_file, line, &(String){7, 6, "/// @ "}, &(_Bool209)) )
  if (!_Bool209) {
  ...
  while (true) {
    ...
    Char _Char210;
    CHECK(208, File_getc(input_file, &(_Char210)) )
    if (_Char210 != ' ') {
      CHECK(209, file_error(name, subname) )
    }
    Bool _Bool211;
    CHECK(210, read_line(input_file, subname, NULL, &(_Bool211)) )
    Bool _Bool212;
    CHECK(211, read_line(expected_output_file, line, NULL, &(_Bool212)) )
    Bool _Bool213;
    CHECK(212, String_equal(line, subname, &(_Bool213)) )
    if (!_Bool213) {
    ...
    if (is_test) {
      Char _Char214;
      CHECK(220, File_getc(input_file, &(_Char214)) )
      if (_Char214 != '-') {
        CHECK(221, file_error(name, subname) )
      }
      Char _Char215;
      CHECK(222, File_getc(input_file, &(_Char215)) )
    ...
    Char _Char216;
    CHECK(242, File_getc(expected_output_file, &(_Char216)) )
    Char _Char217;
    CHECK(243, File_getc(input_file, &(_Char217)) )
    if (!(_Char217 == ' ')) break;
  }
  
  Char _Char218;
  CHECK(245, File_getc(expected_output_file, &(_Char218)) )
  Char _Char219;
  CHECK(246, File_getc(input_file, &(_Char219)) )
  if (_Char219 == EOF) {
    Char _Char220;
    CHECK(247, File_getc(expected_output_file, &(_Char220)) )
    if (_Char220 != EOF) {
...
Returncode read_line(File* file, String* line, String* prefix, Bool* found_prefix) {
  ...
  while (true) {
    ...
    if (NULL != prefix &&  ! (*found_prefix)) {
      Bool _Bool221;
      CHECK(267, String_equal(line, prefix, &(_Bool221)) )
      if (_Bool221) {
        (*found_prefix) = true;
        return OK;
      }
...
Returncode read_data(File* file, String* data, String* prefix) {
  ...
  while (true) {
    Bool _Bool222;
    CHECK(278, read_line(file, line, prefix, &(_Bool222)) )
    if (!(!_Bool222)) break;
    if (data->length > 0) {
...
Returncode test_list() {
  ...
  String* _String223;
  CHECK(297, List_pop(list, (void**)&(_String223)) )
  TEST_ASSERT(297, !(NULL != _String223))
  String* _String224;
  CHECK(298, string_new_copy(&(String){7, 6, "value1"}, &(_String224)) )
  CHECK(298, List_add(list, _String224) )
  String* _String225;
  CHECK(299, string_new_copy(&(String){7, 6, "value2"}, &(_String225)) )
  CHECK(299, List_add(list, _String225) )
  String* _String226;
  CHECK(300, string_new_copy(&(String){7, 6, "value3"}, &(_String226)) )
  CHECK(300, List_add(list, _String226) )
  String* _String227;
  CHECK(301, List_pop(list, (void**)&(_String227)) )
  CHECK(301, f_assert_string(&(String){7, 6, "value1"}, _String227) )
  String* _String228;
  CHECK(302, List_pop(list, (void**)&(_String228)) )
  CHECK(302, f_assert_string(&(String){7, 6, "value2"}, _String228) )
  String* _String229;
  CHECK(303, List_pop(list, (void**)&(_String229)) )
  CHECK(303, f_assert_string(&(String){7, 6, "value3"}, _String229) )
  String* _String230;
  CHECK(304, List_pop(list, (void**)&(_String230)) )
  TEST_ASSERT(304, !(NULL != _String230))
  return OK;
}
#undef LUMI_FUNC_NAME
#endif
...
Returncode test_name_map() {
  ...
  String* _String231;
  CHECK(310, NameMap_find(map, &(String){6, 5, "name1"}, (void**)&(_String231)) )
  TEST_ASSERT(310, !(NULL != _String231))
  CHECK(311, NameMap_add(map, &(String){6, 5, "name1"}, &(String){7, 6, "value1"}) )
  String* _String232;
  CHECK(312, NameMap_find(map, &(String){6, 5, "name1"}, (void**)&(_String232)) )
  CHECK(312, f_assert_string(&(String){7, 6, "value1"}, _String232) )
  String* _String233;
  CHECK(313, NameMap_find(map, &(String){6, 5, "name2"}, (void**)&(_String233)) )
  TEST_ASSERT(313, !(NULL != _String233))
  CHECK(314, NameMap_add(map, &(String){6, 5, "name2"}, &(String){7, 6, "value2"}) )
  String* _String234;
  CHECK(315, NameMap_find(map, &(String){6, 5, "name1"}, (void**)&(_String234)) )
  CHECK(315, f_assert_string(&(String){7, 6, "value1"}, _String234) )
  String* _String235;
  CHECK(316, NameMap_find(map, &(String){6, 5, "name2"}, (void**)&(_String235)) )
  CHECK(316, f_assert_string(&(String){7, 6, "value2"}, _String235) )
  CHECK(317, NameMap_update_or_add(map, &(String){6, 5, "name3"}, &(String){7, 6, "value3"}) )
  String* _String236;
  CHECK(318, NameMap_find(map, &(String){6, 5, "name3"}, (void**)&(_String236)) )
  CHECK(318, f_assert_string(&(String){7, 6, "value3"}, _String236) )
  CHECK(319, NameMap_update_or_add(map, &(String){6, 5, "name2"}, &(String){8, 7, "value2x"}) )
  String* _String237;
  CHECK(320, NameMap_find(map, &(String){6, 5, "name2"}, (void**)&(_String237)) )
  CHECK(320, f_assert_string(&(String){8, 7, "value2x"}, _String237) )
  return OK;
}
#undef LUMI_FUNC_NAME
#endif
...
#define RETURN_ERROR(value) break
    DeclarationArgument* _DeclarationArgument238;
    CHECK(326, (arg)->_base._dtl[5](arg, &(_DeclarationArgument238)) );
#undef RETURN_ERROR
...
#define RETURN_ERROR(value) break
    Char _Char239;
    CHECK(327, (arg)->_base._dtl[6](arg, NULL, &(_Char239)) );
#undef RETURN_ERROR
#define RETURN_ERROR(value) return value
    _trace_stream = stdout;
    TEST_FAIL(327)
...
#undef RETURN_ERROR
#define RETURN_ERROR(value) break
    TypeInstance* _TypeInstance240;
    CHECK(328, (arg)->_base._dtl[7](arg, &(_TypeInstance240)) );
#undef RETURN_ERROR
#define RETURN_ERROR(value) return value
    _trace_stream = stdout;
    TEST_FAIL(328)
...
#undef RETURN_ERROR
#define RETURN_ERROR(value) break
    SyntaxTreeVariable* _SyntaxTreeVariable241;
    CHECK(329, (arg)->_base._dtl[9](arg, &(_SyntaxTreeVariable241)) );
#undef RETURN_ERROR
#define RETURN_ERROR(value) return value
    _trace_stream = stdout;
    TEST_FAIL(329)
...
#undef RETURN_ERROR
#define RETURN_ERROR(value) break
    Expression* _Expression242;
    CHECK(330, (arg)->_base._dtl[10](arg, &(_Expression242)) );
#undef RETURN_ERROR
#define RETURN_ERROR(value) return value
    _trace_stream = stdout;
    TEST_FAIL(330)
...
#undef RETURN_ERROR
#define RETURN_ERROR(value) break
    Argument* _Argument243;
    CHECK(334, (factory)->_dtl[0](factory, &(_Argument243)) );
#undef RETURN_ERROR
#define RETURN_ERROR(value) return value
    _trace_stream = stdout;
    TEST_FAIL(334)
...
#define RETURN_ERROR(value) break
    Char _Char244;
    CHECK(336, (branch)->_base._dtl[6](branch, NULL, &(_Char244)) );
#undef RETURN_ERROR
#define RETURN_ERROR(value) return value
    _trace_stream = stdout;
    TEST_FAIL(336)
...
#define RETURN_ERROR(value) return value
  ...
  _trace_stream = stdout;
  TypeData* _TypeData245;
  CHECK(339, (node)->_dtl[0](node, &(_TypeData245)) )
  TEST_ASSERT(339, !(NULL != _TypeData245))
