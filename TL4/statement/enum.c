...
Returncode EnumData_parse(EnumData* self, Char* end) {
  ...
  Bool _Bool142;
  CHECK(17, f_is_legal_name(self->name, NAME_TYPE, &(_Bool142)) )
  if (!_Bool142) {
    CHECK(18, SyntaxTreeNode_m_syntax_error(&(self->_base), &(String){18, 17, "illegal Enum name"}, self->name) )
  }
  TypeData* _TypeData143;
  CHECK(19, Global_find_type(glob, self->name, &(_TypeData143)) )
  if (NULL != _TypeData143) {
    CHECK(20, SyntaxTreeNode_m_syntax_error(&(self->_base), &(String){25, 24, "Enum name overrides type"}, self->name) )
  }
  EnumData* _EnumData144;
  CHECK(21, NameMap_find(self->my_module->enum_map, self->name, (void**)&(_EnumData144)) )
  if (NULL != _EnumData144) {
  ...
  *self->values = (List){NULL, NULL};
  while (true) {
    ...
    Bool _Bool145;
    CHECK(31, f_is_legal_name(value, NAME_CONSTANT, &(_Bool145)) )
    if (!_Bool145) {
    ...
    String* _String146;
    CHECK(36, string_new_copy(value, &(_String146)) )
    CHECK(36, List_add(self->values, _String146) )
  }
...
Returncode EnumData_m_has_value(EnumData* self, String* value, Bool* has_value) {
  (*has_value) = true;
  Bool _Bool147;
  CHECK(44, String_equal(value, &(String){7, 6, "length"}, &(_Bool147)) )
  if (_Bool147) {
    return OK;
  }
  ...
  while (true) {
    if (!(NULL != node)) break;
    Bool _Bool148;
    CHECK(49, String_equal(value, node->item, &(_Bool148)) )
    if (_Bool148) {
      return OK;
    }
    node = node->next;
