...
Returncode EnumData_parse(EnumData* self, Char* end) {
  ...
  *self->values = (List){NULL, NULL};
  while (true) {
    ...
    Bool _Bool142;
    CHECK(31, f_is_legal_name(value, NAME_CONSTANT, &(_Bool142)) )
    if (!_Bool142) {
    ...
    String* _String143;
    CHECK(36, string_new_copy(value, &(_String143)) )
    CHECK(36, List_add(self->values, _String143) )
  }
...
Returncode EnumData_m_has_value(EnumData* self, String* value, Bool* has_value) {
  (*has_value) = true;
  Bool _Bool144;
  CHECK(44, String_equal(value, &(String){7, 6, "length"}, &(_Bool144)) )
  if (_Bool144) {
    return OK;
  }
  ...
  while (true) {
    if (!(NULL != node)) break;
    Bool _Bool145;
    CHECK(49, String_equal(value, node->item, &(_Bool145)) )
    if (_Bool145) {
      return OK;
    }
    node = node->next;
