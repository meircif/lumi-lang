  ...
  if ((*end) == '{') {
    ...
    *self->parameters = (List){NULL, NULL};
    while (true) {
      ...
      Bool _Bool175;
      CHECK(33, f_is_legal_name(name, NAME_TYPE, &(_Bool175)) )
      if (!_Bool175) {
...
Returncode TypeData_add_type(TypeData* self, ModuleMembers* my_module) {
  Bool _Bool176;
  CHECK(53, f_is_legal_name(self->name, NAME_TYPE, &(_Bool176)) )
  if (!_Bool176) {
    CHECK(54, SyntaxTreeNode_m_syntax_error(&(self->_base._base._base), &(String){18, 17, "illegal type name"}, self->name) )
  }
  TypeData* _TypeData177;
  CHECK(55, Global_find_type(glob, self->name, &(_TypeData177)) )
  if (NULL != _TypeData177) {
    CHECK(56, SyntaxTreeNode_m_syntax_error(&(self->_base._base._base), &(String){21, 20, "redefinition of type"}, self->name) )
  }
  EnumData* _EnumData178;
  CHECK(57, NameMap_find(glob->current_module->enum_map, self->name, (void**)&(_EnumData178)) )
  if (NULL != _EnumData178) {
...
Returncode TypeData_parse_child(TypeData* self, String* keyword, Char* end) {
  Bool _Bool179;
  CHECK(63, SyntaxTreeNamespace_parse_if_function(&(self->_base), keyword, self, &((*end)), &(_Bool179)) )
  if (_Bool179) {
    return OK;
  }
  
  Bool _Bool180;
  CHECK(66, String_equal(keyword, &(String){4, 3, "new"}, &(_Bool180)) )
  if (_Bool180) {
    if ((*end) != '(') {
...
Returncode TypeData_find_field(TypeData* self, String* name, SyntaxTreeVariable** field, Int* bases) {
  ...
  while (true) {
    ...
    while (true) {
      if (!(NULL != child)) break;
      Bool _Bool181;
      CHECK(116, String_equal(((SyntaxTreeVariable*)(child->item))->name, name, &(_Bool181)) )
      if (_Bool181) {
...
Returncode TypeData_find_meth(TypeData* self, String* name, SyntaxTreeFunction** method, Int* bases) {
  ...
  while (true) {
    ...
    while (true) {
      if (!(NULL != child)) break;
      Bool _Bool182;
      CHECK(134, String_equal(((SyntaxTreeFunction*)(child->item))->name, name, &(_Bool182)) )
      if (_Bool182) {
...
Returncode TypeData_analyze(TypeData* self) {
  ...
  Bool _Bool183;
  CHECK(194, List_m_is_empty(self->_base._base.variables, &(_Bool183)) )
  if (!(NULL != self->base_type) && _Bool183) {
...
Returncode TypeData_write_dynamic_init(TypeData* self, TypeData* type_data) {
  ...
  while (true) {
    ...
    SyntaxTreeFunction* method = NULL;
    Int _Int184;
    CHECK(305, TypeData_find_meth(self, ((SyntaxTreeFunction*)(child->value))->name, &(method), &(_Int184)) )
