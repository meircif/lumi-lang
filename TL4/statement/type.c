  ...
  if ((*end) == '{') {
    ...
    *self->parameters = (List){NULL, NULL};
    while (true) {
      ...
      Bool _Bool186;
      CHECK(34, f_is_legal_name(name, NAME_TYPE, &(_Bool186)) )
      if (!_Bool186) {
...
Returncode TypeData_add_type(TypeData* self, ModuleMembers* my_module) {
  Bool _Bool187;
  CHECK(54, f_is_legal_name(self->name, NAME_TYPE, &(_Bool187)) )
  if (!_Bool187) {
    CHECK(55, SyntaxTreeNode_m_syntax_error(&(self->_base._base._base), &(String){18, 17, "illegal type name"}, self->name) )
  }
  TypeData* _TypeData188;
  CHECK(56, Global_find_type(glob, self->name, &(_TypeData188)) )
  if (NULL != _TypeData188) {
    CHECK(57, SyntaxTreeNode_m_syntax_error(&(self->_base._base._base), &(String){21, 20, "redefinition of type"}, self->name) )
  }
  EnumData* _EnumData189;
  CHECK(58, NameMap_find(glob->current_module->enum_map, self->name, (void**)&(_EnumData189)) )
  if (NULL != _EnumData189) {
...
Returncode TypeData_parse_child(TypeData* self, String* keyword, Char* end) {
  Bool _Bool190;
  CHECK(64, SyntaxTreeNamespace_parse_if_function(&(self->_base), keyword, self, &((*end)), &(_Bool190)) )
  if (_Bool190) {
    return OK;
  }
  
  Bool _Bool191;
  CHECK(67, String_equal(keyword, &(String){4, 3, "new"}, &(_Bool191)) )
  if (_Bool191) {
    if ((*end) != '(') {
...
Returncode TypeData_find_field(TypeData* self, String* name, SyntaxTreeVariable** field, Int* bases) {
  ...
  while (true) {
    ...
    while (true) {
      if (!(NULL != child)) break;
      Bool _Bool192;
      CHECK(117, String_equal(((SyntaxTreeVariable*)(child->item))->name, name, &(_Bool192)) )
      if (_Bool192) {
...
Returncode TypeData_find_meth(TypeData* self, String* name, SyntaxTreeFunction** method, Int* bases) {
  ...
  while (true) {
    ...
    while (true) {
      if (!(NULL != child)) break;
      Bool _Bool193;
      CHECK(135, String_equal(((SyntaxTreeFunction*)(child->item))->name, name, &(_Bool193)) )
      if (_Bool193) {
...
Returncode TypeData_analyze(TypeData* self) {
  ...
  Bool _Bool194;
  CHECK(223, List_m_is_empty(self->_base._base.variables, &(_Bool194)) )
  if (!(NULL != self->base_type) && _Bool194) {
...
Returncode TypeData_write_dynamic_init(TypeData* self, TypeData* type_data) {
  ...
  while (true) {
    ...
    SyntaxTreeFunction* method = NULL;
    Int _Int195;
    CHECK(334, TypeData_find_meth(self, ((SyntaxTreeFunction*)(child->value))->name, &(method), &(_Int195)) )
