...
Returncode SyntaxTreeMockFunction_parse(SyntaxTreeMockFunction* self, Char* end) {
  ...
  Bool _Bool177;
  CHECK(170, String_equal(self->mocked_name, &(String){7, 6, "delete"}, &(_Bool177)) )
...
Returncode SyntaxTreeMockFunction_link_types(SyntaxTreeMockFunction* self) {
  ...
  if (NULL != self->type_name) {
    ...
    Bool _Bool178;
    CHECK(183, String_equal(self->mocked_name, &(String){7, 6, "delete"}, &(_Bool178)) )
    if (_Bool178) {
      ...
      Int _Int179;
      CHECK(190, TypeData_find_meth(self->_base.parent_type, self->mocked_name, &(self->mocked_function), &(_Int179)) )
    ...
    Bool _Bool180;
    CHECK(199, String_equal(self->mocked_name, &(String){4, 3, "new"}, &(_Bool180)) )
    if (_Bool180) {
      ...
      Bool _Bool181;
      CHECK(203, String_equal(self->mocked_name, &(String){7, 6, "delete"}, &(_Bool181)) )
      if (_Bool181) {
...
Returncode SyntaxTreeMockFunction_analyze(SyntaxTreeMockFunction* self) {
  glob->current_module = self->_base.my_module;
  Bool _Bool182;
  CHECK(225, String_equal(self->mocked_name, &(String){4, 3, "new"}, &(_Bool182)) )
  if (!(NULL != self->type_name) && _Bool182) {
    ...
    TypeInstance* _TypeInstance183;
    CHECK(231, (((Argument*)(self->_base.arguments->outputs->first->item)))->_base._dtl[7](((Argument*)(self->_base.arguments->outputs->first->item)), &(_TypeInstance183)) )
    if (_TypeInstance183->type_data != glob->type_bool) {
    ...
    Bool _Bool184;
    CHECK(236, String_equal(self->mocked_name, &(String){7, 6, "delete"}, &(_Bool184)) )
    if (_Bool184) {
      self->_base.my_module = NULL;
    }
  }
  if (NULL != self->mocked_function) {
    Bool _Bool185;
    CHECK(239, FunctionArguments_check_same_as(self->_base.arguments, self->mocked_function->arguments, NULL, 0, &(_Bool185)) )
