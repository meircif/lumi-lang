  ...
  if (NULL != self->parameters) {
    ...
    *other->parameters = (List){NULL, NULL};
    ...
    while (true) {
      ...
      CHECK(34, TypeInstance_copy_new(((TypeInstance*)(node->item)), &(_TypeInstance16)) )
      CHECK(34, List_add(other->parameters, _TypeInstance16) )
      node = node->next;
    }
  }
...
Returncode TypeInstance_parse(TypeInstance* self, String* ends, SyntaxTreeNode* node, SyntaxTreeCode* code_node, Int indentation_spaces, Char* end) {
  String* name = NULL;
  String* _String17;
  CHECK(46, string_new_concat(ends, &(String){3, 2, ".{"}, &(_String17)) )
  Int _Int18;
  CHECK(46, read_until(_String17, false, &(name), &((*end)), &(_Int18)) )
  if ((*end) == '.') {
    CHECK(49, string_new_copy(name, &(self->module_name)) )
    String* _String19;
    CHECK(50, string_new_concat(ends, &(String){2, 1, "{"}, &(_String19)) )
    Int _Int20;
    CHECK(50, read_until(_String19, false, &(name), &((*end)), &(_Int20)) )
  }
...
Returncode TypeInstance_parse_name(TypeInstance* self, String* name, SyntaxTreeNode* node, SyntaxTreeCode* code_node, Int indentation_spaces, Char* end) {
  ...
  if ((*end) == '{') {
    Bool _Bool21;
    CHECK(64, String_equal(self->name, &(String){5, 4, "Func"}, &(_Bool21)) )
    if (_Bool21) {
      CHECK(65, read_c(&((*end))) )
      if ((*end) != '(') {
      ...
      *self->arguments = (FunctionArguments){FunctionArguments__dtl, NULL, 0, NULL, NULL};
      ...
      Bool _Bool22;
      CHECK(77, String_equal(self->name, &(String){7, 6, "String"}, &(_Bool22)) )
      if (NULL != code_node && _Bool22) {
        ...
        Bool _Bool23;
        CHECK(83, String_equal(self->name, &(String){6, 5, "Array"}, &(_Bool23)) )
        Bool is_array = _Bool23;
...
Returncode TypeInstance_add_subtype_copy(TypeInstance* self, TypeData* type_data) {
  ...
  TypeInstance* _TypeInstance24;
  CHECK(117, TypeData_m_new_type_instance(type_data, &(_TypeInstance24)) )
  CHECK(117, List_add(self->parameters, _TypeInstance24) )
  return OK;
}
#undef LUMI_FUNC_NAME
#endif
...
Returncode TypeInstance_check_assign_to(TypeInstance* self, Int self_access, TypeInstance* target, Int target_access, SyntaxTreeNode* node, Int* bases) {
  ...
  if (self->type_data == glob->type_generic && target->type_data == glob->type_generic) {
    Bool _Bool25;
    CHECK(232, String_equal(self->name, target->name, &(_Bool25)) )
    if (!_Bool25) {
  ...
  while (true) {
    Bool _Bool26;
    CHECK(241, TypeData_m_is_same(target->type_data, self_type_instance->type_data, &(_Bool26)) )
    if (!(!_Bool26)) break;
  ...
  if (NULL != self->arguments) {
    Bool _Bool27;
    CHECK(255, FunctionArguments_check_same_as(self->arguments, target->arguments, NULL, 0, &(_Bool27)) )
    if (_Bool27) {
...
Returncode TypeInstance_check_sub_assign_to(TypeInstance* self, TypeInstance* target, SyntaxTreeNode* node) {
  ...
  while (true) {
    ...
    Int _Int28;
    CHECK(273, TypeInstance_check_assign_to(((TypeInstance*)(my_sub_type->item)), ACCESS_VAR, target_sub_type->item, ACCESS_VAR, node, &(_Int28)) )
...
Returncode TypeInstance_f_replace_type_parameters(TypeInstance* self, TypeInstance* instance_type, Int bases) {
  ...
  if (self->type_data == glob->type_generic &&  NULL !=  instance_type->parameters &&  NULL !=  instance_type->type_data->parameters) {
    ...
    while (true) {
      ...
      Bool _Bool29;
      CHECK(349, String_equal(((String*)(dec_node->item)), self->name, &(_Bool29)) )
      if (_Bool29) {
