static char* _lumi_file2_name = "map.2.lm";
#define LUMI_FILE_NAME _lumi_file2_name
/* TL3 compiler - attribute mappings */

/* String List */
typedef struct String_list_node String_list_node; struct String_list_node {
  String_list_node* next;
  String* value;
};
static char* _func_name_String_list_node_init = "String-list-node.init";
#define LUMI_FUNC_NAME _func_name_String_list_node_init
Returncode String_list_node_init(String_list_node* self, String* value) {
  self->next = NULL;
  self->value = value;
  return OK;
}
#undef LUMI_FUNC_NAME

typedef struct String_list String_list; struct String_list {
  String_list_node* first;
  String_list_node* last;
};
static char* _func_name_String_list_init = "String-list.init";
#define LUMI_FUNC_NAME _func_name_String_list_init
Returncode String_list_init(String_list* self) {
  self->first = NULL;
  self->last = NULL;
  return OK;
}
#undef LUMI_FUNC_NAME
static char* _func_name_String_list_has = "String-list.has";
#define LUMI_FUNC_NAME _func_name_String_list_has
Returncode String_list_has(String_list* self, Bool* res) {
  (*res) = NULL != self->first;
  return OK;
}
#undef LUMI_FUNC_NAME
static char* _func_name_String_list_add = "String-list.add";
#define LUMI_FUNC_NAME _func_name_String_list_add
Returncode String_list_add(String_list* self, String* value) {
  String_list_node* node = malloc(sizeof(String_list_node));
  if (node == NULL) RAISE(24)
  CHECK(25, String_list_node_init(node, value));
  if (NULL != self->last) {
    self->last->next = node;
  }
  else {
    self->first = node;
  }
  self->last = node;
  return OK;
}
#undef LUMI_FUNC_NAME

static char* _func_name_f_init_new_string_list = "f-init-new-string-list";
#define LUMI_FUNC_NAME _func_name_f_init_new_string_list
Returncode f_init_new_string_list(String_list** new_list) {
  String_list* list = malloc(sizeof(String_list));
  if (list == NULL) RAISE(33)
  CHECK(34, String_list_init(list));
  (*new_list) = list;
  return OK;
}
#undef LUMI_FUNC_NAME


/* Name Map */
typedef struct Name_map_node Name_map_node; struct Name_map_node {
  Name_map_node* next;
  String* name;
  Object* value;
};
static char* _func_name_Name_map_node_init = "Name-map-node.init";
#define LUMI_FUNC_NAME _func_name_Name_map_node_init
Returncode Name_map_node_init(Name_map_node* self, String* name, Object* value, Name_map_node* next) {
  self->next = next;
  self->name = name;
  self->value = value;
  return OK;
}
#undef LUMI_FUNC_NAME

typedef struct Name_map Name_map; struct Name_map {
  Name_map_node* first;
  Name_map_node* last;
  Name_map_node* extension;
};
static char* _func_name_Name_map_init = "Name-map.init";
#define LUMI_FUNC_NAME _func_name_Name_map_init
Returncode Name_map_init(Name_map* self) {
  self->first = NULL;
  self->last = NULL;
  self->extension = NULL;
  return OK;
}
#undef LUMI_FUNC_NAME
static char* _func_name_Name_map_add = "Name-map.add";
#define LUMI_FUNC_NAME _func_name_Name_map_add
Returncode Name_map_add(Name_map* self, String* name, Object* value) {
  Name_map_node* node = malloc(sizeof(Name_map_node));
  if (node == NULL) RAISE(60)
  CHECK(61, Name_map_node_init(node, name, value, self->extension));
  if (NULL != self->last) {
    self->last->next = node;
  }
  else {
    self->first = node;
  }
  self->last = node;
  return OK;
}
#undef LUMI_FUNC_NAME
static char* _func_name_Name_map_find = "Name-map.find";
#define LUMI_FUNC_NAME _func_name_Name_map_find
Returncode Name_map_find(Name_map* self, String* name, Object** value) {
  Name_map_node* node = self->first;
  while (true) {
    if (!(NULL != node)) break;
    Bool _Bool0;
    CHECK(72, String_equal(node->name, name, &(_Bool0)))
    if (_Bool0) {
      (*value) = node->value;
      return OK;
    }
    node = node->next;
  }
  (*value) = NULL;
  return OK;
}
#undef LUMI_FUNC_NAME
static char* _func_name_Name_map_print = "Name-map.print";
#define LUMI_FUNC_NAME _func_name_Name_map_print
Returncode Name_map_print(Name_map* self) {
  Name_map_node* node = self->first;
  while (true) {
    if (!(NULL != node)) break;
    CHECK(82, Sys_print(sys, node->name));
    node = node->next;
  }
  return OK;
}
#undef LUMI_FUNC_NAME


typedef struct Mvar Mvar;

typedef struct Var_map Var_map; struct Var_map {
  Name_map _base;
};
static char* _func_name_Var_map_m_copy_from = "Var-map.m-copy-from";
#define LUMI_FUNC_NAME _func_name_Var_map_m_copy_from
Returncode Var_map_m_copy_from(Var_map* self, Var_map* other) {
  self->_base.first = other->_base.first;
  self->_base.extension = other->_base.first;
  return OK;
}
#undef LUMI_FUNC_NAME
static char* _func_name_Var_map_add = "Var-map.add";
#define LUMI_FUNC_NAME _func_name_Var_map_add
Returncode Var_map_add(Var_map* self, String* name, Mvar* mvar) {
  Object* value = mvar;
  CHECK(95, Name_map_add(&(self->_base), name, value));
  return OK;
}
#undef LUMI_FUNC_NAME
static char* _func_name_Var_map_find = "Var-map.find";
#define LUMI_FUNC_NAME _func_name_Var_map_find
Returncode Var_map_find(Var_map* self, String* name, Mvar** mvar) {
  Object* _Object1;
  CHECK(98, Name_map_find(&(self->_base), name, &(_Object1)))
  (*mvar) = _Object1;
  return OK;
}
#undef LUMI_FUNC_NAME

typedef struct Var_map_iter Var_map_iter; struct Var_map_iter {
  Name_map_node* node;
};
static char* _func_name_Var_map_iter_init = "Var-map-iter.init";
#define LUMI_FUNC_NAME _func_name_Var_map_iter_init
Returncode Var_map_iter_init(Var_map_iter* self, Var_map* map) {
  self->node = map->_base.first;
  return OK;
}
#undef LUMI_FUNC_NAME
static char* _func_name_Var_map_iter_get_name = "Var-map-iter.get-name";
#define LUMI_FUNC_NAME _func_name_Var_map_iter_get_name
Returncode Var_map_iter_get_name(Var_map_iter* self, String** name) {
  (*name) = self->node->name;
  return OK;
}
#undef LUMI_FUNC_NAME
static char* _func_name_Var_map_iter_get_var = "Var-map-iter.get-var";
#define LUMI_FUNC_NAME _func_name_Var_map_iter_get_var
Returncode Var_map_iter_get_var(Var_map_iter* self, Mvar** mvar) {
  Mvar* value = self->node->value;
  (*mvar) = value;
  return OK;
}
#undef LUMI_FUNC_NAME
static char* _func_name_Var_map_iter_m_has_data = "Var-map-iter.m-has-data";
#define LUMI_FUNC_NAME _func_name_Var_map_iter_m_has_data
Returncode Var_map_iter_m_has_data(Var_map_iter* self, Bool* has_data) {
  (*has_data) = NULL != self->node;
  return OK;
}
#undef LUMI_FUNC_NAME
static char* _func_name_Var_map_iter_m_next = "Var-map-iter.m-next";
#define LUMI_FUNC_NAME _func_name_Var_map_iter_m_next
Returncode Var_map_iter_m_next(Var_map_iter* self) {
  self->node = self->node->next;
  return OK;
}
#undef LUMI_FUNC_NAME


static char* _func_name_f_copy_new_var_map = "f-copy-new-var-map";
#define LUMI_FUNC_NAME _func_name_f_copy_new_var_map
Returncode f_copy_new_var_map(Var_map* other, Var_map** new_var_map) {
  Var_map* var_map = malloc(sizeof(Var_map));
  if (var_map == NULL) RAISE(121)
  CHECK(122, Name_map_init(&(var_map->_base)));
  if (NULL != other) {
    CHECK(124, Var_map_m_copy_from(var_map, other));
  }
  (*new_var_map) = var_map;
  return OK;
}
#undef LUMI_FUNC_NAME


typedef struct Mtype Mtype; struct Mtype {
  String* name;
  Var_map* members;
  Var_map* dynamic_members;
  String* base_typename;
  Mtype* base_mtype;
  Mtype* generic_mtype;
  String* default_value;
  Bool is_primitive;
};
static char* _func_name_Mtype_init = "Mtype.init";
#define LUMI_FUNC_NAME _func_name_Mtype_init
Returncode Mtype_init(Mtype* self, String* name) {
  self->name = name;
  CHECK(140, f_copy_new_var_map(NULL, &(self->members)));
  self->dynamic_members = NULL;
  self->base_typename = NULL;
  self->base_mtype = NULL;
  self->generic_mtype = NULL;
  self->default_value = NULL;
  self->is_primitive = false;
  return OK;
}
#undef LUMI_FUNC_NAME
static char* _func_name_Mtype_set_primitive = "Mtype.set-primitive";
#define LUMI_FUNC_NAME _func_name_Mtype_set_primitive
Returncode Mtype_set_primitive(Mtype* self, String* default_value) {
  self->is_primitive = true;
  CHECK(150, f_new_copy(default_value, &(self->default_value)));
  return OK;
}
#undef LUMI_FUNC_NAME


Int ACCESS_COPY = 0;
Int ACCESS_USER = 1;
Int ACCESS_OWNER = 2;
Int ACCESS_VAR = 3;

typedef struct Mfunc Mfunc;

Returncode f_find_type(String* typename, Mtype** mtype);

typedef struct Mvar Mvar; struct Mvar {
  String* name;
  String* typename;
  Mtype* mtype;
  String* sub_typename;
  Mtype* sub_mtype;
  Mfunc* func_dec;
  Int access;
  Bool is_ref;
};
static char* _func_name_Mvar_init = "Mvar.init";
#define LUMI_FUNC_NAME _func_name_Mvar_init
Returncode Mvar_init(Mvar* self, Mtype* mtype) {
  self->name = NULL;
  self->typename = NULL;
  self->mtype = mtype;
  self->sub_typename = NULL;
  self->sub_mtype = NULL;
  self->func_dec = NULL;
  self->access = ACCESS_VAR;
  self->is_ref = false;
  if (NULL != mtype) {
    CHECK(182, f_new_copy(mtype->name, &(self->typename)));
    if (NULL != mtype->generic_mtype) {
      CHECK(184, f_new_copy(mtype->generic_mtype->name, &(self->sub_typename)));
      self->sub_mtype = mtype->generic_mtype;
    }
  }
  return OK;
}
#undef LUMI_FUNC_NAME
static char* _func_name_Mvar_analyze = "Mvar.analyze";
#define LUMI_FUNC_NAME _func_name_Mvar_analyze
Returncode Mvar_analyze(Mvar* self) {
  if (!(NULL != self->mtype)) {
    CHECK(189, f_find_type(self->typename, &(self->mtype)));
  }
  if (NULL != self->sub_typename && !(NULL != self->sub_mtype)) {
    CHECK(191, f_find_type(self->sub_typename, &(self->sub_mtype)));
  }
  return OK;
}
#undef LUMI_FUNC_NAME

static char* _func_name_init_new_var = "init-new-var";
#define LUMI_FUNC_NAME _func_name_init_new_var
Returncode init_new_var(Mtype* mtype, Mvar** new_mvar) {
  Mvar* mvar = malloc(sizeof(Mvar));
  if (mvar == NULL) RAISE(194)
  CHECK(195, Mvar_init(mvar, mtype));
  (*new_mvar) = mvar;
  return OK;
}
#undef LUMI_FUNC_NAME

static char* _func_name_add_var = "add-var";
#define LUMI_FUNC_NAME _func_name_add_var
Returncode add_var(Var_map* map, Mvar* mvar) {
  CHECK(199, Var_map_add(map, mvar->name, mvar));
  return OK;
}
#undef LUMI_FUNC_NAME


/* Operator Map */
typedef struct Op_map Op_map; struct Op_map {
  Name_map _base;
};
static char* _func_name_Op_map_add = "Op-map.add";
#define LUMI_FUNC_NAME _func_name_Op_map_add
Returncode Op_map_add(Op_map* self, String* name, String* ctext) {
  String* new_name;
  CHECK(206, f_new_copy(name, &(new_name)));
  String* new_text;
  CHECK(208, f_new_copy(ctext, &(new_text)));
  Object* value = new_text;
  CHECK(210, Name_map_add(&(self->_base), new_name, value));
  return OK;
}
#undef LUMI_FUNC_NAME
static char* _func_name_Op_map_add_copy = "Op-map.add-copy";
#define LUMI_FUNC_NAME _func_name_Op_map_add_copy
Returncode Op_map_add_copy(Op_map* self, String* name) {
  CHECK(213, Op_map_add(self, name, name));
  return OK;
}
#undef LUMI_FUNC_NAME
static char* _func_name_Op_map_find = "Op-map.find";
#define LUMI_FUNC_NAME _func_name_Op_map_find
Returncode Op_map_find(Op_map* self, String* name, String** ctext) {
  Object* _Object2;
  CHECK(216, Name_map_find(&(self->_base), name, &(_Object2)))
  (*ctext) = _Object2;
  return OK;
}
#undef LUMI_FUNC_NAME


/* Type Map */
typedef struct Type_map Type_map; struct Type_map {
  Name_map _base;
};
static char* _func_name_Type_map_add = "Type-map.add";
#define LUMI_FUNC_NAME _func_name_Type_map_add
Returncode Type_map_add(Type_map* self, String* name, Mtype** out_mtype) {
  Mtype* mtype = malloc(sizeof(Mtype));
  if (mtype == NULL) RAISE(222)
  String* new_name;
  CHECK(224, f_new_copy(name, &(new_name)));
  CHECK(225, Mtype_init(mtype, new_name));
  Object* value = mtype;
  CHECK(227, Name_map_add(&(self->_base), new_name, value));
  (*out_mtype) = mtype;
  return OK;
}
#undef LUMI_FUNC_NAME
static char* _func_name_Type_map_find = "Type-map.find";
#define LUMI_FUNC_NAME _func_name_Type_map_find
Returncode Type_map_find(Type_map* self, String* name, Mtype** mtype) {
  Object* _Object3;
  CHECK(231, Name_map_find(&(self->_base), name, &(_Object3)))
  (*mtype) = _Object3;
  return OK;
}
#undef LUMI_FUNC_NAME


/* Node Map */
typedef struct St St;

typedef struct Node_map_item Node_map_item; struct Node_map_item {
  String* key;
  Type node_type;
};

#undef LUMI_FILE_NAME
