...
Returncode write_func(St_func* st_func) {
  ...
  CHECK(write(&(String){3, 2, "\";"}))
  CHECK(write_new_indent_line())
  CHECK(write(&(String){35, 34, "#define LUMI_FUNC_NAME _func_name_"}))
  CHECK(write_func_name(st_func))
  CHECK(write_new_indent_line())
  ...
  CHECK(write_block_close())
  CHECK(write_new_indent_line())
  CHECK(write(&(String){23, 22, "#undef LUMI_FUNC_NAME\n"}))
  return OK;
}
...
Returncode f_compile_file(Array* argv, Int index) {
  ...
  if (index == argv->length - 1) {
    CHECK(write(&(String){21, 20, "#include \"lumi.2.h\"\n"}))
    Int index = 1;
    while (true) {
...
Returncode func(Array* argv) {
  CHECK(print(&(String){20, 19, "Lumi compiler start"}))
  
  if (argv->length < 2) {
    CHECK(print(&(String){42, 41, "usage: tl2-compiler [input Lumi files]..."}))
    return OK;
  }
  /* init global data */
  ...
  CHECK(print(&(String){18, 17, "Lumi compiler end"}))
  return OK;
}
