...
Returncode St_node_parse_block(St_node* self) {
  ...
  while (true) {
    Bool _Bool3;
    CHECK(120, St_node_parse_line(self, spaces, &(_Bool3)))
    if (!(_Bool3)) break;
  }
  return OK;
}
...
Returncode St_node_parse(St_node* self) {
  ...
  CHECK(126, St_node_parse_block(self));
...
Returncode St_node_analyze_first(St_node* self) {
  CHECK(130, St_analyze_first(&(self->_base)));
  return OK;
}
#undef MR_FUNC_NAME
...
Returncode St_node_analyze(St_node* self) {
  CHECK(133, St_analyze(&(self->_base)));
  return OK;
}
#undef MR_FUNC_NAME
...
Returncode St_node_write_block(St_node* self) {
  ...
  while (true) {
    ...
    if (!(NULL != glob->mclass)) {
      CHECK(141, write_spaces());
    }
    CHECK(142, (*((Func**)(son)))[3](son));
    if (!(NULL != glob->mclass)) {
...
Returncode St_node_write_indent_block(St_node* self) {
  Int my_spaces = glob->spaces;
  glob->spaces = my_spaces + 2;
  CHECK(151, St_node_write_block(self));
  glob->spaces = my_spaces;
...
Returncode St_node_write(St_node* self) {
  ...
  CHECK(159, St_node_write_indent_block(self));
  return OK;
}
#undef MR_FUNC_NAME
...
Returncode St_file_init(St_file* self, St* root, Array* argv, Int index) {
  CHECK(168, St_init(&(self->_base._base), root));
  if ((index) < 0 || (index) >= argv->length) RAISE(169)
...
Returncode St_file_parse(St_file* self) {
  Bool _Bool4;
  CHECK(174, String_has(self->infile_name, '"', &(_Bool4)))
  if (_Bool4) {
    CHECK(175, f_msg_raise(&(String){32, 31, "Illegal \" character in argument"}, self->infile_name));
  }
  if (self->infile_name->length < 6) {
    CHECK(177, f_msg_raise(&(String){19, 18, "too short argument"}, self->infile_name));
  }
  CHECK(178, file_open_read(self->infile_name, &(glob->infile)));
  ...
  CHECK(185, St_node_parse_block(&(self->_base)));
  
  CHECK(187, File_close(glob->infile));
  return OK;
}
#undef MR_FUNC_NAME
...
Returncode St_file_analyze_first(St_file* self) {
  ...
  CHECK(192, St_node_analyze_first(&(self->_base)));
  return OK;
}
#undef MR_FUNC_NAME
...
Returncode St_file_analyze(St_file* self) {
  glob->infile_name = self->infile_name;
  glob->node = &(self->_base._base);
  CHECK(197, St_node_analyze(&(self->_base)));
  return OK;
}
#undef MR_FUNC_NAME
...
Returncode St_file_write_cfile_name(St_file* self, Int index) {
  if ((index) < 0 || (index) >= self->argv->length) RAISE(200)
  String* infile_name = ((String*)(self->argv->values)) + index;
  if ((0) < 0 || (infile_name->length - 4) < 0 || (0) + (infile_name->length - 4) > infile_name->length) RAISE(201)
  String* prefix = &(String){infile_name->length - 4, infile_name->length - 4, infile_name->chars + (0)};
  CHECK(202, write(&(String){3, 2, " \""}));
  CHECK(203, write(prefix));
  CHECK(204, write(&(String){4, 3, "c\"\n"}));
...
Returncode St_file_write(St_file* self) {
  ...
  if (outfile_name == NULL) RAISE(208)
  if ((0) < 0 || (self->infile_name->length - 4) < 0 || (0) + (self->infile_name->length - 4) > self->infile_name->length) RAISE(209)
  CHECK(209, String_copy(outfile_name, &(String){self->infile_name->length - 4, self->infile_name->length - 4, self->infile_name->chars + (0)}));
  CHECK(210, String_append(outfile_name, 'c'));
  CHECK(211, file_open_write(outfile_name, &(glob->outfile)));
  
  CHECK(213, write(&(String){21, 20, "#ifndef MR_MAINFILE\n"}));
  CHECK(214, write(&(String){20, 19, "#define MR_MAINFILE"}));
  CHECK(215, St_file_write_cfile_name(self, self->index));
  CHECK(216, write(&(String){15, 14, "#define DEPTH "}));
  CHECK(217, write_int(glob->max_type_depth + 1));
  CHECK(218, write(&(String){20, 19, "\n#include \"mr.3.h\"\n"}));
  CHECK(219, write(&(String){8, 7, "#else\n\n"}));
  
  CHECK(221, write(&(String){29, 28, "#if MR_STAGE == MR_TYPEDEFS\n"}));
  CHECK(222, write(&(String){22, 21, "static char* _mr_file"}));
  CHECK(223, write_int(self->index));
  CHECK(224, write(&(String){10, 9, "_name = \""}));
  CHECK(225, write(self->infile_name));
  CHECK(226, write(&(String){4, 3, "\";\n"}));
  CHECK(227, write(&(String){8, 7, "#endif\n"}));
  CHECK(228, write(&(String){30, 29, "#define MR_FILE_NAME _mr_file"}));
  CHECK(229, write_int(self->index));
  CHECK(230, write(&(String){8, 7, "_name\n\n"}));
  glob->infile_name = self->infile_name;
  
  CHECK(233, St_node_write_block(&(self->_base)));
  
  CHECK(235, write(&(String){23, 22, "\n#undef MR_FILE_NAME\n\n"}));
  
  if (self->argv->length > 2) {
    CHECK(238, write(&(String){21, 20, "#ifndef MR_INCLUDES\n"}));
    CHECK(239, write(&(String){21, 20, "#define MR_INCLUDES\n"}));
    Int n; for (n = 1; n < self->argv->length; ++n) {
      if (n != self->index) {
        CHECK(242, write(&(String){9, 8, "#include"}));
        CHECK(243, St_file_write_cfile_name(self, n));
      }
    }
  ...
  Int depth; for (depth = 0; depth < glob->max_type_depth; ++depth) {
    if (depth == 0) {
      CHECK(247, write(&(String){5, 4, "#if "}));
    }
    else {
      CHECK(249, write(&(String){7, 6, "#elif "}));
    }
    CHECK(250, write(&(String){22, 21, "MR_STAGE == MR_TYPES("}));
    CHECK(251, write_int(depth + 1));
    CHECK(252, write(&(String){3, 2, ")\n"}));
    CHECK(253, write(&(String){17, 16, "#undef MR_STAGE\n"}));
    CHECK(254, write(&(String){27, 26, "#define MR_STAGE MR_TYPES("}));
    CHECK(255, write_int(depth + 2));
    CHECK(256, write(&(String){3, 2, ")\n"}));
    if (depth == glob->max_type_depth - 1) {
      CHECK(258, write(&(String){8, 7, "#endif\n"}));
    }
    
  }
  if (self->argv->length > 2) {
    CHECK(261, write(&(String){20, 19, "#undef MR_INCLUDES\n"}));
    CHECK(262, write(&(String){9, 8, "#endif\n\n"}));
  }
  
  CHECK(264, write(&(String){8, 7, "#endif\n"}));
  
  CHECK(266, File_close(glob->outfile));
  free(outfile_name);
  return OK;
}
#undef MR_FUNC_NAME
...
Returncode St_root_init(St_root* self, Array* argv) {
  CHECK(274, St_init(&(self->_base), NULL));
  self->argv = argv;
  return OK;
}
#undef MR_FUNC_NAME
...
Returncode St_root_parse(St_root* self) {
  Int index; for (index = 1; index < self->argv->length; ++index) {
    ...
    if (st_file == NULL) RAISE(279)
    *((Func**)(st_file)) = St_file__dtl;
    CHECK(280, St_file_init(st_file, &(self->_base), self->argv, index));
    CHECK(281, (*((Func**)(st_file)))[0](st_file));
  }
  return OK;
}
#undef MR_FUNC_NAME
...
Returncode St_root_analyze_first(St_root* self) {
  CHECK(284, St_analyze_first(&(self->_base)));
  return OK;
}
#undef MR_FUNC_NAME
...
Returncode St_root_analyze(St_root* self) {
  CHECK(287, (*((Func**)(self)))[1](self));
  CHECK(288, St_analyze(&(self->_base)));
  return OK;
}
#undef MR_FUNC_NAME
...
Returncode St_native_parse(St_native* self) {
  CHECK(300, St_comment_parse(&(self->_base)));
  return OK;
}
#undef MR_FUNC_NAME
